...
{
	{ "Up",               false, "D-PAD ВВЕРХ",           ":/help/dpad_up.svg" },
	{ "Down",             false, "D-PAD ВНИЗ",         ":/help/dpad_down.svg" },
	{ "Left",             false, "D-PAD ВЛЕВО",         ":/help/dpad_left.svg" },
	{ "Right",            false, "D-PAD ВПРАВО",        ":/help/dpad_right.svg" },
	...
	{ "LeftShoulder",     true,  "LEFT SHOULDER L10",      ":/help/button_l.svg" },
	{ "RightShoulder",    true,  "RIGHT SHOULDER R10",     ":/help/button_r.svg" },
	{ "LeftTrigger",      true,  "LEFT TRIGGER L11",       ":/help/button_lt.svg" },
	{ "RightTrigger",     true,  "RIGHT TRIGGER R11",      ":/help/button_rt.svg" },
	{ "LeftThumb",        true,  "LEFT THUMB L12",         ":/help/analog_thumb.svg" },
	{ "RightThumb",       true,  "RIGHT THUMB R12",        ":/help/analog_thumb.svg" },
	...
	{ "HotKeyEnable",     true,  "ГОРЯЧАЯ КНОПКА",      ":/help/button_hotkey.svg" }
};
	...
	mTargetConfig(target), mHoldingInput(false), mBusyAnim(window)
{
	...
	mTitle = std::make_shared<TextComponent>(mWindow, "НАСТРОЙКА", Font::get(FONT_SIZE_LARGE), 0x555555FF, ALIGN_CENTER);
	...
	mSubtitle2 = std::make_shared<TextComponent>(mWindow, "УДЕРЖИВАЙТЕ ЛЮБУЮ КНОПКУ ЧТОБЫ ПРОПУСТИТЬ", Font::get(FONT_SIZE_SMALL), 0x99999900, ALIGN_CENTER);
	...
	for(int i = 0; i < inputCount; i++)
	{
		...
		auto mapping = std::make_shared<TextComponent>(mWindow, "-НЕ НАЗНАЧЕНО-", Font::get(FONT_SIZE_MEDIUM, FONT_PATH_LIGHT), 0x999999FF, ALIGN_RIGHT);
	...
	// make the first one say "НАЖМИТЕ ЛЮБУЮ КНОПКУ" if we're re-configuring everything
	if(mConfiguringAll)
	...
	if(mConfiguringRow && mHoldingInput && GUI_INPUT_CONFIG_LIST[mHeldInputId].skippable)
	{
		...
		if(mHeldTime >= HOLD_TO_SKIP_MS)
		{
			...
			if(prevSec != curSec)
			{
				...
				std::stringstream ss;
				ss << "УДЕРЖИВАТЬ НА ПРОТЯЖЕНИИ " << HOLD_TO_SKIP_MS/1000 - curSec << "S ЧТОБЫ ПРОПУСТИТЬ";
				text->setText(ss.str());
