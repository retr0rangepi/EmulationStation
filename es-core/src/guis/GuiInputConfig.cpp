...
static const InputConfigStructure GUI_INPUT_CONFIG_LIST[inputCount] =
{
	...
	{ "LeftShoulder",     true,  "LEFT SHOULDER L9",      ":/help/button_l.svg" },
	{ "RightShoulder",    true,  "RIGHT SHOULDER R9",     ":/help/button_r.svg" },
	{ "LeftTrigger",      true,  "LEFT TRIGGER L10",       ":/help/button_lt.svg" },
	{ "RightTrigger",     true,  "RIGHT TRIGGER R10",      ":/help/button_rt.svg" },
	{ "LeftThumb",        true,  "LEFT THUMB L11",         ":/help/analog_thumb.svg" },
	{ "RightThumb",       true,  "RIGHT THUMB R11",        ":/help/analog_thumb.svg" },
	...
	{ "HotKeyEnable",     true,  "Játék kilépés",      ":/help/button_hotkey.svg" }
};
	...
	mTargetConfig(target), mHoldingInput(false), mBusyAnim(window)
{
	...
	mTitle = std::make_shared<TextComponent>(mWindow, "Konfigurálás", Font::get(FONT_SIZE_LARGE), 0x555555FF, ALIGN_CENTER);
	...
	mSubtitle2 = std::make_shared<TextComponent>(mWindow, "Tartsd valamelyik gombot nyomva a kihagyáshoz.", Font::get(FONT_SIZE_SMALL), 0x99999900, ALIGN_CENTER);
	...
	for(int i = 0; i < inputCount; i++)
	{
		...
		auto mapping = std::make_shared<TextComponent>(mWindow, "-Nincs definiálva-", Font::get(FONT_SIZE_MEDIUM, FONT_PATH_LIGHT), 0x999999FF, ALIGN_RIGHT);
	...
	// make the first one say "Nyomj meg egy gombot" if we're re-configuring everything
	if(mConfiguringAll)
