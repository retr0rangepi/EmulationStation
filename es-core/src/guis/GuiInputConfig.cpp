...
static const InputConfigStructure GUI_INPUT_CONFIG_LIST[inputCount] =
{
	...
	{ "LeftShoulder",     true,  "LEFT SHOULDER L1",      ":/help/button_l.svg" },
	{ "RightShoulder",    true,  "RIGHT SHOULDER R1",     ":/help/button_r.svg" },
	{ "LeftTrigger",      true,  "LEFT TRIGGER L2",       ":/help/button_lt.svg" },
	{ "RightTrigger",     true,  "RIGHT TRIGGER R2",      ":/help/button_rt.svg" },
	{ "LeftThumb",        true,  "LEFT THUMB L3",         ":/help/analog_thumb.svg" },
	{ "RightThumb",       true,  "RIGHT THUMB R3",        ":/help/analog_thumb.svg" },
	...
	{ "HotKeyEnable",     true,  "Activar el Hotkey",      ":/help/button_hotkey.svg" }
};
	...
	mTargetConfig(target), mHoldingInput(false), mBusyAnim(window)
{
	...
	mSubtitle2 = std::make_shared<TextComponent>(mWindow, "Mantén cualquier botón para saltar", Font::get(FONT_SIZE_SMALL), 0x99999900, ALIGN_CENTER);
	...
	// make the first one say "PULSE CUALQUIERA" if we're re-configuring everything
	if(mConfiguringAll)
	...
	if(mConfiguringRow && mHoldingInput && GUI_INPUT_CONFIG_LIST[mHeldInputId].skippable)
	{
		...
		if(mHeldTime >= HOLD_TO_SKIP_MS)
		{
			...
			if(prevSec != curSec)
			{
				...
				std::stringstream ss;
				ss << "Mantenido para" << HOLD_TO_SKIP_MS/1000 - curSec << "S para saltar";
				text->setText(ss.str());
...
{
	text->setText("Actualmente ocupado");
	text->setColor(0x656565FF);
}
