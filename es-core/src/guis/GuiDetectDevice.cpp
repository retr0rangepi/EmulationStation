	...
	mBackground(window, ":/frame.png"), mGrid(window, Vector2i(1, 5))
{
	...
	// title
	mTitle = std::make_shared<TextComponent>(mWindow, firstRun ? "VELKOMMEN" : "Konfigurer Input", 
	...
	if(numDevices > 0)
		...
		deviceInfo << "Ingen GAMEPADS fundet";
	...
	// message
	mMsg1 = std::make_shared<TextComponent>(mWindow, "Hold en knap inde på din enhed for at konfigurere det", Font::get(FONT_SIZE_SMALL), 0x777777FF, ALIGN_CENTER);
	mGrid.setEntry(mMsg1, Vector2i(0, 2), false, true);

	const char* msg2str = firstRun ? "tryk F4 for at afslutte når som helst" : "Tryk ESC for at afbryde";
