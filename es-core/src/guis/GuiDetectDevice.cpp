	...
	mBackground(window, ":/frame.png"), mGrid(window, Vector2i(1, 5))
{
	...
	// title
	mTitle = std::make_shared<TextComponent>(mWindow, firstRun ? "BIENVENIDO" : "Configurar método de entrada", 
	...
	if(numDevices > 0)
		...
		deviceInfo << "Ningún control detectado";
	...
	// message
	mMsg1 = std::make_shared<TextComponent>(mWindow, "Mantén un botón en tu control para designarlo.", Font::get(FONT_SIZE_SMALL), 0x777777FF, ALIGN_CENTER);
	mGrid.setEntry(mMsg1, Vector2i(0, 2), false, true);

	const char* msg2str = firstRun ? "PRESIONA F4 PARA CANCELAR EN CUALQUIER MOMENTO" : "Presiona ESC para cancelar";
