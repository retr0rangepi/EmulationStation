	...
	mBackground(window, ":/frame.png"), mGrid(window, Vector2i(1, 5))
{
	...
	// title
	mTitle = std::make_shared<TextComponent>(mWindow, firstRun ? "BIENVENU" : "CONFIGURATION DES TOUCHES", 
	...
	if(numDevices > 0)
		...
		deviceInfo << "AUCUNES MANETTES DÉTECTÉES";
	...
	// message
	mMsg1 = std::make_shared<TextComponent>(mWindow, "MAINTENEZ UN BOUTON ENFONCÉ SUR VOTRE APPAREIL POUR LE CONFIGURER", Font::get(FONT_SIZE_SMALL), 0x777777FF, ALIGN_CENTER);
	mGrid.setEntry(mMsg1, Vector2i(0, 2), false, true);

	const char* msg2str = firstRun ? "APPUYEZ SUR F4 POUR QUITTER À TOUS MOMENT." : "APPUYEZ SUR ÉCHAP POUR ANNULER";
