	...
	mBackground(window, ":/frame.png"), mGrid(window, Vector2i(1, 5))
{
	...
	// title
	mTitle = std::make_shared<TextComponent>(mWindow, firstRun ? "WILLKOMMEN" : "KONFIGURIERE EINGABE", 
	...
	if(numDevices > 0)
		...
		deviceInfo << "KEINE GAMEPADS ERKANNT";
	...
	// message
	mMsg1 = std::make_shared<TextComponent>(mWindow, "BUTTON AUF DEM CONTROLLER DRÜCKEN UM IHN EINZURICHTEN", Font::get(FONT_SIZE_SMALL), 0x777777FF, ALIGN_CENTER);
	mGrid.setEntry(mMsg1, Vector2i(0, 2), false, true);

	const char* msg2str = firstRun ? "DRÜCKE JEDERZEIT F4 ZUM BEENDEN" : "ZUM ABBRECHEN ESC DRÜCKEN";
