	...
	mBackground(window, ":/frame.png"), mGrid(window, Vector2i(1, 5))
{
	...
	// title
	mTitle = std::make_shared<TextComponent>(mWindow, firstRun ? "BEM VINDO" : "CONFIGURAR CONTROLES", 
	...
	if(numDevices > 0)
		...
		deviceInfo << "NENHUM CONTROLE DETECTADO";
	...
	// message
	mMsg1 = std::make_shared<TextComponent>(mWindow, "PRESSIONE UM BOTÃO PARA CONFIGURAR SEU DISPOSITIVO.", Font::get(FONT_SIZE_SMALL), 0x777777FF, ALIGN_CENTER);
	mGrid.setEntry(mMsg1, Vector2i(0, 2), false, true);

	const char* msg2str = firstRun ? "PRESSIONE F4 PARA SAIR" : "PRESSIONE ESC PARA CANCELAR.";
