	...
	mBackground(window, ":/frame.png"), mGrid(window, Vector2i(1, 5))
{
	...
	// title
	mTitle = std::make_shared<TextComponent>(mWindow, firstRun ? "BENVENTUO" : "CONFIGURA INPUT", 
	...
	if(numDevices > 0)
		...
		deviceInfo << "NESSUN GAMEPAD RILEVATO";
	...
	// message
	mMsg1 = std::make_shared<TextComponent>(mWindow, "TIENI PREMUTO UN TASTO SULLA PERIFERICA DA CONFIGURARE", Font::get(FONT_SIZE_SMALL), 0x777777FF, ALIGN_CENTER);
	mGrid.setEntry(mMsg1, Vector2i(0, 2), false, true);

	const char* msg2str = firstRun ? "PREMI F4 PER USCIRE" : "PREMI ESC PER ANNULLARE";
