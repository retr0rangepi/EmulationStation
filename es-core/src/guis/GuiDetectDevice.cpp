	...
	mBackground(window, ":/frame.png"), mGrid(window, Vector2i(1, 5))
{
	...
	// title
	mTitle = std::make_shared<TextComponent>(mWindow, firstRun ? "ДОБРО ПОЖАЛОВАТЬ" : "НАСТРОИТЬ КОНТРОЛЛЕР", 
	...
	if(numDevices > 0)
		...
		deviceInfo << "КОНТРОЛЛЕРОВ НЕ ОБНАРУЖЕНО";
	...
	// message
	mMsg1 = std::make_shared<TextComponent>(mWindow, "УДЕРЖИВАЙТЕ ЛЮБУЮ КНОПКУ НА КОНРОЛЛЕРЕ ДЛЯ НАЧАЛА НАСТРОЙКИ", Font::get(FONT_SIZE_SMALL), 0x777777FF, ALIGN_CENTER);
	mGrid.setEntry(mMsg1, Vector2i(0, 2), false, true);

	const char* msg2str = firstRun ? "НАЖМИТЕ НА КЛАВИАТУРЕ F4 ЧТОБЫ ВЫЙТИ В ЛЮБОЕ ВРЕМЯ" : "НАЖМИТЕ ESC ДЛЯ ОТМЕНЫ";
