	...
	mBackground(window, ":/frame.png"), mGrid(window, Vector2i(1, 5))
{
	...
	// message
	mMsg1 = std::make_shared<TextComponent>(mWindow, "按住手柄任意鍵進行設置", Font::get(FONT_SIZE_SMALL), 0x777777FF, ALIGN_CENTER);
	mGrid.setEntry(mMsg1, Vector2i(0, 2), false, true);

	const char* msg2str = firstRun ? "PRESS F4 TO QUIT AT ANY TIME." : "按ESC鍵取消設置";
