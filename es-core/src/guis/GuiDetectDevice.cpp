	...
	mBackground(window, ":/frame.png"), mGrid(window, Vector2i(1, 5))
{
	...
	// title
	mTitle = std::make_shared<TextComponent>(mWindow, firstRun ? "BIENVENIDO" : "CONFIGURAR CONTROLES", 
	...
	if(numDevices > 0)
		...
		deviceInfo << "NO SE HA DETECTADO NINGUN MANDO";
	...
	// message
	mMsg1 = std::make_shared<TextComponent>(mWindow, "PULSE UN BOTON EN SU DISPOSITIVO PARA CONFIGURARLO.", Font::get(FONT_SIZE_SMALL), 0x777777FF, ALIGN_CENTER);
	mGrid.setEntry(mMsg1, Vector2i(0, 2), false, true);

	const char* msg2str = firstRun ? "PULSE F4 PARA SALIR EN CUALQUIER MOMENTO." : "PRESIONE ESC PARA CANCELAR.";
