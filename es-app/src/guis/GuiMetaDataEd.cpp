	...
	mSavedCallback(saveCallback), mDeleteFunc(deleteFunc)
{
	...
	mTitle = std::make_shared<TextComponent>(mWindow, "MODIFICA DATI", Font::get(FONT_SIZE_LARGE), 0x555555FF, ALIGN_CENTER);
	...
	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "SALVA", "Salva", [&] { save(); delete this; }));
	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "ANNULLA", "Annulla", [&] { delete this; }));
	...
	if(mDeleteFunc)
	{
		...
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "CANCELLA", "Cancella", deleteBtnFunc));
	}
