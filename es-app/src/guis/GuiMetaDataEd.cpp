	...
	mSavedCallback(saveCallback), mDeleteFunc(deleteFunc)
{
	...
	mTitle = std::make_shared<TextComponent>(mWindow, "Játék adatai", Font::get(FONT_SIZE_LARGE), 0x555555FF, ALIGN_CENTER);
	...
	if(!scraperParams.system->hasPlatformId(PlatformIds::PLATFORM_IGNORE))
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "Letöltés", "Letöltés", std::bind(&GuiMetaDataEd::fetch, this)));

	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "Mentés", "Mentés", [&] { save(); delete this; }));
	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "Mégse", "Mégse", [&] { delete this; }));

	if(mDeleteFunc)
	{
		auto deleteFileAndSelf = [&] { mDeleteFunc(); delete this; };
		auto deleteBtnFunc = [this, deleteFileAndSelf] { mWindow->pushGui(new GuiMsgBox(mWindow, "THIS WILL DELETE A FILE!\nARE YOU SURE?", "Igen", deleteFileAndSelf, "Nem", nullptr)); };
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "Törlés", "Törlés", deleteBtnFunc));
	}
...
void GuiMetaDataEd::close(bool closeAllWindows)
{
	...
	if(dirty)
	{
		...
		mWindow->pushGui(new GuiMsgBox(mWindow,
			"Elmented a beállítások?",
			"Igen", [this, closeFunc] { save(); closeFunc(); },
			"Nem", closeFunc
		));
	}else{
		closeFunc();
