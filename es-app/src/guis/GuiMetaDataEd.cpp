	...
	mSavedCallback(saveCallback), mDeleteFunc(deleteFunc)
{
	...
	mTitle = std::make_shared<TextComponent>(mWindow, "ИЗМЕНИТЬ ОПИСАНИЕ", Font::get(FONT_SIZE_LARGE), 0x555555FF, ALIGN_CENTER);
	...
	if(!scraperParams.system->hasPlatformId(PlatformIds::PLATFORM_IGNORE))
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "ЗАГРУЗИТЬ ОПИСАНИЕ", "загрузить описание", std::bind(&GuiMetaDataEd::fetch, this)));

	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "СОХРАНИТЬ", "сохранить", [&] { save(); delete this; }));
	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "ОТМЕНА", "отмена", [&] { delete this; }));

	if(mDeleteFunc)
	{
		auto deleteFileAndSelf = [&] { mDeleteFunc(); delete this; };
		auto deleteBtnFunc = [this, deleteFileAndSelf] { mWindow->pushGui(new GuiMsgBox(mWindow, "THIS WILL DELETE A FILE!\nARE YOU SURE?", "ДА", deleteFileAndSelf, "НЕТ", nullptr)); };
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "УДАЛИТЬ", "удалить", deleteBtnFunc));
	}
...
void GuiMetaDataEd::close(bool closeAllWindows)
{
	...
	if(dirty)
	{
		...
		mWindow->pushGui(new GuiMsgBox(mWindow,
			"СОХРАНИТЬ ИЗМЕНЕНИЯ?",
			"ДА", [this, closeFunc] { save(); closeFunc(); },
			"НЕТ", closeFunc
		));
	}else{
		closeFunc();
