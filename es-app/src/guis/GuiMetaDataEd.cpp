	...
	mSavedCallback(saveCallback), mDeleteFunc(deleteFunc)
{
	...
	mTitle = std::make_shared<TextComponent>(mWindow, " EDITIERE METADATEN", Font::get(FONT_SIZE_LARGE), 0x555555FF, ALIGN_CENTER);
	...
	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "SPEICHERN", "speichern", [&] { save(); delete this; }));
	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "ABBRECHEN", "abbrechen", [&] { delete this; }));

	if(mDeleteFunc)
	{
		auto deleteFileAndSelf = [&] { mDeleteFunc(); delete this; };
		auto deleteBtnFunc = [this, deleteFileAndSelf] { mWindow->pushGui(new GuiMsgBox(mWindow, "THIS WILL DELETE A FILE!\nARE YOU SURE?", "JA", deleteFileAndSelf, "NEIN", nullptr)); };
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "LÖSCHEN", "löschen", deleteBtnFunc));
	}
...
void GuiMetaDataEd::close(bool closeAllWindows)
{
	...
	if(dirty)
	{
		...
		mWindow->pushGui(new GuiMsgBox(mWindow,
			...
			"JA", [this, closeFunc] { save(); closeFunc(); },
			"NEIN", closeFunc
		));
	}else{
		closeFunc();
