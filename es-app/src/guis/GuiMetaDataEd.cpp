	...
	mSavedCallback(saveCallback), mDeleteFunc(deleteFunc)
{
	...
	if(!scraperParams.system->hasPlatformId(PlatformIds::PLATFORM_IGNORE))
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "BUSCAR METADATOS", "buscar metadatos", std::bind(&GuiMetaDataEd::fetch, this)));
	...
	if(mDeleteFunc)
	{
		...
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "ELIMINAR", "eliminar", deleteBtnFunc));
	}
