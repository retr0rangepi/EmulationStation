	...
	mSavedCallback(saveCallback), mDeleteFunc(deleteFunc)
{
	...
	mTitle = std::make_shared<TextComponent>(mWindow, "編輯遊戲資料", Font::get(FONT_SIZE_LARGE), 0x555555FF, ALIGN_CENTER);
	...
	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "儲存", "儲存", [&] { save(); delete this; }));
	...
	if(mDeleteFunc)
	{
		...
		auto deleteBtnFunc = [this, deleteFileAndSelf] { mWindow->pushGui(new GuiMsgBox(mWindow, "THIS WILL DELETE A FILE!\nARE YOU SURE?", "是", deleteFileAndSelf, "否", nullptr)); };
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "刪除", "刪除", deleteBtnFunc));
	}
