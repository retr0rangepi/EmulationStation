	...
	mClose(false)
{
	...
	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "GİR", "search", [&] {
		mSearch->openInputScreen(mSearchParams);
		mGrid.resetCursor();
	}));
	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "İPTAL", "cancel", [&] { delete this; }));
