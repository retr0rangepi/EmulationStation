	...
	mClose(false)
{
	...
	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "ENTRÉE", "search", [&] {
		mSearch->openInputScreen(mSearchParams);
		mGrid.resetCursor();
	}));
	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "ANNULER", "cancel", [&] { delete this; }));
