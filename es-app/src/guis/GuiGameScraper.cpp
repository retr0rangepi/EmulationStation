	...
	mClose(false)
{
	...
	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "ВВОД", "search", [&] {
		mSearch->openInputScreen(mSearchParams);
		mGrid.resetCursor();
	}));
	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "ОТМЕНА", "cancel", [&] { delete this; }));
