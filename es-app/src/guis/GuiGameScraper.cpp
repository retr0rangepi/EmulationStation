	...
	mClose(false)
{
	...
	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "EINGABE", "search", [&] {
		mSearch->openInputScreen(mSearchParams);
		mGrid.resetCursor();
	}));
	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "ABBRECHEN", "cancel", [&] { delete this; }));
