	...
	mSystem(system), mMenu(window, "OPCIONES"), fromPlaceholder(false), mFiltersChanged(false)
{
	...
	// check it's not a placeholder folder - if it is, only show "Opciones del filtro"
	...
	{
		row.elements.clear();
		row.addElement(std::make_shared<TextComponent>(mWindow, "Agregar/Remover juegos a esa colección", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	{
		row.elements.clear();
		row.addElement(std::make_shared<TextComponent>(mWindow, "Editar el metadato de este juego", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
		row.addElement(makeArrow(mWindow), false);
