...
GuiGamelistOptions::GuiGamelistOptions(Window* window, SystemData* system) : GuiComponent(window),
	mSystem(system), mMenu(window, "OPZIONI"), fromPlaceholder(false), mFiltersChanged(false)
{
	addChild(&mMenu);

	// check it's not a placeholder folder - if it is, only show "Opzioni Filtro"
	...
	if (!fromPlaceholder) {
		...
		// sort list by
		mListSort = std::make_shared<SortList>(mWindow, "ORDINA GIOCHI PER", false);
		...
		mMenu.addWithLabel("ORDINA GIOCHI PER", mListSort);
	}
	// show filtered menu
	if(!Settings::getInstance()->getBool("ForceDisableFilters"))
	{
		row.elements.clear();
		row.addElement(std::make_shared<TextComponent>(mWindow, "FILTRA LISTA GIOCHI", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	{
		row.elements.clear();
		row.addElement(std::make_shared<TextComponent>(mWindow, "AGGIUNGI/RIMUOVI GIOCHI A COLLEZIONE", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	{
		row.elements.clear();
		row.addElement(std::make_shared<TextComponent>(mWindow, "MODIFICA I DATI DI QUESTO GIOCO", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
		row.addElement(makeArrow(mWindow), false);
