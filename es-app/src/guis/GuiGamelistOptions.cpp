...
GuiGamelistOptions::GuiGamelistOptions(Window* window, SystemData* system) : GuiComponent(window),
	mSystem(system), mMenu(window, "SEÇENEKLER"), fromPlaceholder(false), mFiltersChanged(false)
{
	addChild(&mMenu);

	// check it's not a placeholder folder - if it is, only show "Seçenekleri Filtrele"
	...
	if (!fromPlaceholder) {
		...
		// sort list by
		mListSort = std::make_shared<SortList>(mWindow, "OYUNLARI ŞU ŞEKİLDE SIRALA", false);
		...
		mMenu.addWithLabel("OYUNLARI ŞU ŞEKİLDE SIRALA", mListSort);
	}
	// show filtered menu
	if(!Settings::getInstance()->getBool("ForceDisableFilters"))
	{
		row.elements.clear();
		row.addElement(std::make_shared<TextComponent>(mWindow, "OYUN LİSTESİNİ FİLTRELE", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	{
		row.elements.clear();
		row.addElement(std::make_shared<TextComponent>(mWindow, "OYUNLARI BU OYUN KOLEKSİYONUNDAN EKLE/KALDIR", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	{
		row.elements.clear();
		row.addElement(std::make_shared<TextComponent>(mWindow, "BU OYUNUN KILAVUZ VERİSİNİ DÜZENLE", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
		row.addElement(makeArrow(mWindow), false);
