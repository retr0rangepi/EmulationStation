...
GuiCollectionSystemsOptions::GuiCollectionSystemsOptions(Window* window) : GuiComponent(window), mMenu(window, "НАСТРОЙКА КОЛЛЕКЦИЙ ИГР")
{
	initializeMenu();
}
...
void GuiCollectionSystemsOptions::initializeMenu()
{
	...
	{
		addEntry("СОЗДАТЬ НОВУЮ КОЛЛЕКЦИЮ ИЗ ТЕМЫ", 0x777777FF, true,
		[this, unusedFolders] {
			auto s = new GuiSettings(mWindow, "ВЫБРАТЬ ПАПКУ ТЕМЫ");
			std::shared_ptr< OptionListComponent<std::string> > folderThemes = std::make_shared< OptionListComponent<std::string> >(mWindow, "ВЫБРАТЬ ПАПКУ ТЕМЫ", true);
	...
	ComponentListRow row;
	row.addElement(std::make_shared<TextComponent>(mWindow, "СОЗДАТЬ НОВУЮ КОЛЛЕКЦИЮ", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	mMenu.addWithLabel("ГРУППИРОВАТЬ КОЛЛЕКЦИИ БЕЗ ТЕМЫ", bundleCustomCollections);
	...
	mMenu.addWithLabel("СОРТИРОВАТЬ КОЛЛЕКЦИИ И СИСТЕМЫ", sortAllSystemsSwitch);
	...
	mMenu.addButton("НАЗАД", "back", std::bind(&GuiCollectionSystemsOptions::applySettings, this));
...
void GuiCollectionSystemsOptions::addSystemsToMenu()
{
	...
	autoOptionList = std::make_shared< OptionListComponent<std::string> >(mWindow, "ВЫБРАТЬ КОЛЛЕКЦИИ", true);
	...
	mMenu.addWithLabel("АВТОМАТИЧЕСКИЕ КОЛЛЕКЦИИ", autoOptionList);

	std::map<std::string, CollectionSystemData> customSystems = CollectionSystemManager::get()->getCustomCollectionSystems();

	customOptionList = std::make_shared< OptionListComponent<std::string> >(mWindow, "ВЫБРАТЬ КОЛЛЕКЦИИ", true);
	...
	mMenu.addWithLabel("ПОЛЬЗОВАТЕЛЬСКИЕ КОЛЛЕКЦИИ", customOptionList);
}
