...
GuiCollectionSystemsOptions::GuiCollectionSystemsOptions(Window* window) : GuiComponent(window), mMenu(window, "AJUSTES DE COLEÇÕES DE JOGOS")
{
	initializeMenu();
}
...
void GuiCollectionSystemsOptions::initializeMenu()
{
	...
	{
		addEntry("CRIAR UMA NOVA COLEÇÃO DE JOGOS BASEADO NO TEMA", 0x777777FF, true,
		[this, unusedFolders] {
			auto s = new GuiSettings(mWindow, "ESCOLHER PASTA DO TEMA");
			std::shared_ptr< OptionListComponent<std::string> > folderThemes = std::make_shared< OptionListComponent<std::string> >(mWindow, "ESCOLHER PASTA DO TEMA", true);
	...
	ComponentListRow row;
	row.addElement(std::make_shared<TextComponent>(mWindow, "CRIAR COLEÇÃO PERSONALIZADA", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	mMenu.addWithLabel("AGRUPAR COLEÇÕES SEM TEMA", bundleCustomCollections);
	...
	mMenu.addWithLabel("ORDENAR COLEÇÕES PERSONALIZADAS E SISTEMAS", sortAllSystemsSwitch);
	...
	mMenu.addButton("RETORNA", "back", std::bind(&GuiCollectionSystemsOptions::applySettings, this));
...
void GuiCollectionSystemsOptions::addSystemsToMenu()
{
	...
	autoOptionList = std::make_shared< OptionListComponent<std::string> >(mWindow, "SELECIONA COLEÇÕES", true);
	...
	mMenu.addWithLabel("COLEÇÕES AUTOMÁTICAS", autoOptionList);

	std::map<std::string, CollectionSystemData> customSystems = CollectionSystemManager::get()->getCustomCollectionSystems();

	customOptionList = std::make_shared< OptionListComponent<std::string> >(mWindow, "SELECIONA COLEÇÕES", true);
	...
	mMenu.addWithLabel("COLEÇÕES PERSONALIZADAS", customOptionList);
}
