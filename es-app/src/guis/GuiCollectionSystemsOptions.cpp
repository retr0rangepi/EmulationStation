...
GuiCollectionSystemsOptions::GuiCollectionSystemsOptions(Window* window) : GuiComponent(window), mMenu(window, "PARAMÈTRES DE LA COLLECTION DE JEU")
{
	initializeMenu();
}
...
void GuiCollectionSystemsOptions::initializeMenu()
{
	...
	{
		addEntry("CREER UNE NOUVELLE COLLECTION PERSONNALISÉE À PARTIR D´UN THÈME", 0x777777FF, true,
		[this, unusedFolders] {
			auto s = new GuiSettings(mWindow, "SÉLECTIONNER LE DOSSIER DU THÈME");
			std::shared_ptr< OptionListComponent<std::string> > folderThemes = std::make_shared< OptionListComponent<std::string> >(mWindow, "SÉLECTIONNER LE DOSSIER DU THÈME", true);
	...
	ComponentListRow row;
	row.addElement(std::make_shared<TextComponent>(mWindow, "CRÉER UNE NOUVELLE COLLECTION PERSONNALISÉE", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	mMenu.addWithLabel("GROUPER LES COLLECTIONS PERSONNALISÉES SANS THÈME", bundleCustomCollections);
	...
	mMenu.addWithLabel("TRIER LA COLLECTION ET LES SYSTÈMES PERSONNALISÉS", sortAllSystemsSwitch);
	...
	mMenu.addButton("RETOUR", "back", std::bind(&GuiCollectionSystemsOptions::applySettings, this));
...
void GuiCollectionSystemsOptions::addSystemsToMenu()
{
	...
	autoOptionList = std::make_shared< OptionListComponent<std::string> >(mWindow, "SELECTIONNEZ UNE COLLECTIONS", true);
	...
	mMenu.addWithLabel("COLLECTIONS DE JEU AUTOMATIQUE", autoOptionList);

	std::map<std::string, CollectionSystemData> customSystems = CollectionSystemManager::get()->getCustomCollectionSystems();

	customOptionList = std::make_shared< OptionListComponent<std::string> >(mWindow, "SELECTIONNEZ UNE COLLECTIONS", true);
	...
	mMenu.addWithLabel("COLLECTIONS DE JEUX PERSONNALISÉES", customOptionList);
}
