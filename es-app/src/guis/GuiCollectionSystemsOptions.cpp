...
GuiCollectionSystemsOptions::GuiCollectionSystemsOptions(Window* window) : GuiComponent(window), mMenu(window, "AJUSTES DE LOS JUEGOS")
{
	initializeMenu();
}
...
void GuiCollectionSystemsOptions::initializeMenu()
{
	...
	{
		addEntry("CREAR UNA NUEVA COLECCIÓN PERSONALIZADA DE UN TEMA", 0x777777FF, true,
		[this, unusedFolders] {
			auto s = new GuiSettings(mWindow, "SELECCIONE CARPETA DE TEMA");
			std::shared_ptr< OptionListComponent<std::string> > folderThemes = std::make_shared< OptionListComponent<std::string> >(mWindow, "SELECCIONE CARPETA DE TEMA", true);
	...
	ComponentListRow row;
	row.addElement(std::make_shared<TextComponent>(mWindow, "CREAR UNA NUEVA COLECCION PERSONALIZADA", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	mMenu.addWithLabel("AGRUPAR COLECCIONES PERSONALIZADAS SIN TEMA", bundleCustomCollections);
	...
	mMenu.addWithLabel("ORDENAR COLECCIONES PERSONALIZADAS Y SISTEMAS", sortAllSystemsSwitch);
...
void GuiCollectionSystemsOptions::addSystemsToMenu()
{
	...
	autoOptionList = std::make_shared< OptionListComponent<std::string> >(mWindow, "SELECCIONAR COLECCION", true);
	...
	mMenu.addWithLabel("COLECCIONES DE JUEGOS AUTOMATICA", autoOptionList);

	std::map<std::string, CollectionSystemData> customSystems = CollectionSystemManager::get()->getCustomCollectionSystems();

	customOptionList = std::make_shared< OptionListComponent<std::string> >(mWindow, "SELECCIONAR COLECCION", true);
	...
	mMenu.addWithLabel("COLECCIONES DE JUEGOS PERSONALIZADAS", customOptionList);
}
