...
GuiCollectionSystemsOptions::GuiCollectionSystemsOptions(Window* window) : GuiComponent(window), mMenu(window, "OYUN KOLEKSİYONU AYARLARI")
{
	initializeMenu();
}
...
void GuiCollectionSystemsOptions::initializeMenu()
{
	...
	{
		addEntry("TEMADAN YENİ ÖZEL KOLEKSİYON OLUŞTUR", 0x777777FF, true,
		[this, unusedFolders] {
			auto s = new GuiSettings(mWindow, "TEMA KLASÖRÜ SEÇ");
			std::shared_ptr< OptionListComponent<std::string> > folderThemes = std::make_shared< OptionListComponent<std::string> >(mWindow, "TEMA KLASÖRÜ SEÇ", true);
	...
	ComponentListRow row;
	row.addElement(std::make_shared<TextComponent>(mWindow, "YENİ ÖZEL KOLEKSİYON OLUŞTUR", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	mMenu.addWithLabel("TEMASIZ ÖZEL KOLEKSİYONLARI GRUPLA", bundleCustomCollections);
	...
	mMenu.addWithLabel("ÖZEL KOLEKSİYONLARI VE SİSTEMLERİ SIRALA", sortAllSystemsSwitch);
	...
	mMenu.addButton("GERİ", "back", std::bind(&GuiCollectionSystemsOptions::applySettings, this));
...
void GuiCollectionSystemsOptions::addSystemsToMenu()
{
	...
	autoOptionList = std::make_shared< OptionListComponent<std::string> >(mWindow, "KOLEKSİYONLARI SEÇİN", true);
	...
	mMenu.addWithLabel("OTOMATİK OYUN KOLEKSİYONLARI", autoOptionList);

	std::map<std::string, CollectionSystemData> customSystems = CollectionSystemManager::get()->getCustomCollectionSystems();

	customOptionList = std::make_shared< OptionListComponent<std::string> >(mWindow, "KOLEKSİYONLARI SEÇİN", true);
	...
	mMenu.addWithLabel("ÖZEL OYUN KOLEKSİYONLARI", customOptionList);
}
