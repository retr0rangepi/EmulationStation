...
GuiCollectionSystemsOptions::GuiCollectionSystemsOptions(Window* window) : GuiComponent(window), mMenu(window, "Spil samling indstillinger")
{
	initializeMenu();
}
...
void GuiCollectionSystemsOptions::initializeMenu()
{
	...
	{
		addEntry("Skab ny Custom samling ud fra Tema", 0x777777FF, true,
		[this, unusedFolders] {
			auto s = new GuiSettings(mWindow, "Vælg tema mappe");
			std::shared_ptr< OptionListComponent<std::string> > folderThemes = std::make_shared< OptionListComponent<std::string> >(mWindow, "Vælg tema mappe", true);
	...
	ComponentListRow row;
	row.addElement(std::make_shared<TextComponent>(mWindow, "skab ny Custom samling ud fra Tema", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	mMenu.addWithLabel("Grupper Custom samlinger uden Tema", bundleCustomCollections);
	...
	mMenu.addWithLabel("Sorter Custom samlinger og systmer", sortAllSystemsSwitch);
	...
	mMenu.addButton("Retur", "back", std::bind(&GuiCollectionSystemsOptions::applySettings, this));
...
void GuiCollectionSystemsOptions::addSystemsToMenu()
{
	...
	autoOptionList = std::make_shared< OptionListComponent<std::string> >(mWindow, "vælg samlinger", true);
	...
	mMenu.addWithLabel("Automatiske Spil samlinger", autoOptionList);

	std::map<std::string, CollectionSystemData> customSystems = CollectionSystemManager::get()->getCustomCollectionSystems();

	customOptionList = std::make_shared< OptionListComponent<std::string> >(mWindow, "vælg samlinger", true);
	...
	mMenu.addWithLabel("Custom spil samlinger", customOptionList);
}
