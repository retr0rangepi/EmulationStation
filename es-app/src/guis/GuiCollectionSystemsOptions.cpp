...
GuiCollectionSystemsOptions::GuiCollectionSystemsOptions(Window* window) : GuiComponent(window), mMenu(window, "游戏收藏设置")
{
	initializeMenu();
}
...
void GuiCollectionSystemsOptions::initializeMenu()
{
	...
	{
		addEntry("从主题创建自定义收藏", 0x777777FF, true,
		[this, unusedFolders] {
	...
	ComponentListRow row;
	row.addElement(std::make_shared<TextComponent>(mWindow, "创建自定义收藏", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	mMenu.addWithLabel("自定义组收藏", bundleCustomCollections);
	...
	mMenu.addWithLabel("排序自定义收藏和主机", sortAllSystemsSwitch);
...
void GuiCollectionSystemsOptions::addSystemsToMenu()
{
	...
	autoOptionList = std::make_shared< OptionListComponent<std::string> >(mWindow, "选择收藏", true);
	...
	mMenu.addWithLabel("自动游戏收藏", autoOptionList);

	std::map<std::string, CollectionSystemData> customSystems = CollectionSystemManager::get()->getCustomCollectionSystems();

	customOptionList = std::make_shared< OptionListComponent<std::string> >(mWindow, "选择收藏", true);
	...
	mMenu.addWithLabel("自定义游戏收藏", customOptionList);
}
