...
GuiCollectionSystemsOptions::GuiCollectionSystemsOptions(Window* window) : GuiComponent(window), mMenu(window, "SETTINGS COLLEZIONI")
{
	initializeMenu();
}
...
void GuiCollectionSystemsOptions::initializeMenu()
{
	...
	{
		addEntry("CREA COLLEZIONE DA TEMA", 0x777777FF, true,
		[this, unusedFolders] {
			auto s = new GuiSettings(mWindow, "SELEZIONA CARTELLA TEMA");
			std::shared_ptr< OptionListComponent<std::string> > folderThemes = std::make_shared< OptionListComponent<std::string> >(mWindow, "SELEZIONA CARTELLA TEMA", true);
	...
	ComponentListRow row;
	row.addElement(std::make_shared<TextComponent>(mWindow, "CREA COLLEZIONE PERSONALIZZATA", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	mMenu.addWithLabel("RAGGRUPPA COLLEZIONE SENZA TEMA", bundleCustomCollections);
	...
	mMenu.addWithLabel("ORDINA COLLEZIONI E SISTEMI", sortAllSystemsSwitch);
	...
	mMenu.addButton("INDIETRO", "back", std::bind(&GuiCollectionSystemsOptions::applySettings, this));
...
void GuiCollectionSystemsOptions::addSystemsToMenu()
{
	...
	autoOptionList = std::make_shared< OptionListComponent<std::string> >(mWindow, "SELEZIONA COLLEZIONI", true);
	...
	mMenu.addWithLabel("COLLEZIONI GIOCHI AUTOMATICHE", autoOptionList);

	std::map<std::string, CollectionSystemData> customSystems = CollectionSystemManager::get()->getCustomCollectionSystems();

	customOptionList = std::make_shared< OptionListComponent<std::string> >(mWindow, "SELEZIONA COLLEZIONI", true);
	...
	mMenu.addWithLabel("COLLEZIONI PERSONALIZZATE", customOptionList);
}
