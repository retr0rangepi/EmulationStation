...
GuiCollectionSystemsOptions::GuiCollectionSystemsOptions(Window* window) : GuiComponent(window), mMenu(window, "AJUSTES DE COLECCIONES DE JUEGOS")
{
	initializeMenu();
}
...
void GuiCollectionSystemsOptions::initializeMenu()
{
	...
	ComponentListRow row;
	row.addElement(std::make_shared<TextComponent>(mWindow, "CREAR UNA NUEVA COLECCIÓN PERSONALIZADA", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	mMenu.addButton("ATRÁS", "back", std::bind(&GuiCollectionSystemsOptions::applySettings, this));
...
void GuiCollectionSystemsOptions::addSystemsToMenu()
{
	...
	mMenu.addWithLabel("COLECCIONES DE JUEGOS AUTOMATICAS", autoOptionList);
