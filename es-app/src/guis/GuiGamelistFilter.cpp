...
GuiGamelistFilter::GuiGamelistFilter(Window* window, SystemData* system) : GuiComponent(window), mMenu(window, "Filtrar lista de juegos por"), mSystem(system)
{
	initializeMenu();
}
...
void GuiGamelistFilter::initializeMenu()
{
	...
	// show filtered menu
	row.elements.clear();
	row.addElement(std::make_shared<TextComponent>(mWindow, "Reiniciar todos los filtros", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
