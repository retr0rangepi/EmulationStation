...
GuiGamelistFilter::GuiGamelistFilter(Window* window, SystemData* system) : GuiComponent(window), mMenu(window, "Filter spil liste efter"), mSystem(system)
{
	initializeMenu();
}
...
void GuiGamelistFilter::initializeMenu()
{
	...
	// show filtered menu
	row.elements.clear();
	row.addElement(std::make_shared<TextComponent>(mWindow, "Nulstil alle filtre", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
