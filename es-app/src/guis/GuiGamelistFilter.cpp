...
GuiGamelistFilter::GuiGamelistFilter(Window* window, SystemData* system) : GuiComponent(window), mMenu(window, "FILTER LA LISTE DE JEU PAR"), mSystem(system)
{
	initializeMenu();
}
...
void GuiGamelistFilter::initializeMenu()
{
	...
	// show filtered menu
	row.elements.clear();
	row.addElement(std::make_shared<TextComponent>(mWindow, "REINITIALISER TOUS LES FILTRES", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
