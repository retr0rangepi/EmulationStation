...
GuiGamelistFilter::GuiGamelistFilter(Window* window, SystemData* system) : GuiComponent(window), mMenu(window, "OYUN LİSTESİNİ ŞU ŞEKİLDE FİLTRELE"), mSystem(system)
{
	initializeMenu();
}
...
void GuiGamelistFilter::initializeMenu()
{
	...
	// show filtered menu
	row.elements.clear();
	row.addElement(std::make_shared<TextComponent>(mWindow, "TÜM FİLTRELERİ TEMİZLE", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
