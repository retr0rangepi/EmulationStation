...
GuiMenu::GuiMenu(Window* window) : GuiComponent(window), mMenu(window, "ROPi 4.2 FULL"), mVersion(window)
{
	...
	if (!(UIModeController::getInstance()->isUIModeKid() && Settings::getInstance()->getBool("hideQuitMenuOnKidUI")))
		addEntry("SAIR DO EMULATIONSTATION", 0x777777FF, true, [this] {openQuitMenu(); });

        if (isFullUI) addEntry("ÁREA DE TRABALHO - DESKTOP", 0x777777FF, true, [this] { Window* window = mWindow;
                window->pushGui(new GuiMsgBox(window, "Você gostaria de entrar no modo Desktop?", "Sim", [window]
                        { system("startx 2> /dev/null"); }, "Não", nullptr) ); });

	if (isFullUI)
		addEntry("CONFIGURAR CONTROLES-TECLADO", 0x777777FF, true, [this] { openConfigInput(); });

	if (isFullUI)
		addEntry("BUSCADOR - SCRAPER", 0x777777FF, true, [this] { openScraperSettings(); });

        if (isFullUI)
		addEntry("AJUSTES DE SOM", 0x777777FF, true, [this] { openSoundSettings(); });

	if (isFullUI)
		addEntry("AJUSTES DE INTERFACE", 0x777777FF, true, [this] { openUISettings(); });

	if (isFullUI)
		addEntry("AJUSTE DA COLEÇÃO DE JOGOS", 0x777777FF, true, [this] { openCollectionSystemSettings(); });

        if (isFullUI) addEntry("SLEEP MODE", 0x777777FF, true, [this] { Window* window = mWindow;
                window->pushGui(new GuiMsgBox(window, "REALLY SLEEP?", "Sim", [window]
                        { system("/home/pi/RetrOrangePi/Power_Button/Sleep_mode.sh 2> /dev/null"); }, "Não", nullptr) ); });

	if (isFullUI)
		addEntry("OUTROS AJUSTES", 0x777777FF, true, [this] { openOtherSettings(); });
...
void GuiMenu::openScraperSettings()
{
	...
	// scrape from
	auto scraper_list = std::make_shared< OptionListComponent< std::string > >(mWindow, "BAIXAR DE", false);
	std::vector<std::string> scrapers = getScraperList();
	for(auto it = scrapers.cbegin(); it != scrapers.cend(); it++)
		scraper_list->add(*it, *it, *it == Settings::getInstance()->getString("Scraper"));

	s->addWithLabel("BAIXAR DE", scraper_list);
	s->addSaveFunc([scraper_list] { Settings::getInstance()->setString("Scraper", scraper_list->getSelected()); });

	// scrape ratings
	auto scrape_ratings = std::make_shared<SwitchComponent>(mWindow);
	scrape_ratings->setState(Settings::getInstance()->getBool("ScrapeRatings"));
	s->addWithLabel("BAIXAR AVALIAÇÕES", scrape_ratings);
	...
	auto scrape_now = std::make_shared<TextComponent>(mWindow, "BAIXAR AGORA", Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
...
{
	auto s = new GuiSettings(mWindow, "AJUSTES DE SOM");
	...
	if (UIModeController::getInstance()->isUIModeFull())
	{
		...
		s->addWithLabel("ATIVAR AUDIO DOS VIDEOS", video_audio);
...
void GuiMenu::openUISettings()
{
	...
	//UI mode
	auto UImodeSelection = std::make_shared< OptionListComponent<std::string> >(mWindow, "INTERFACE", false);
	std::vector<std::string> UImodes = UIModeController::getInstance()->getUIModes();
	for (auto it = UImodes.cbegin(); it != UImodes.cend(); it++)
		UImodeSelection->add(*it, *it, Settings::getInstance()->getString("UIMode") == *it);
	s->addWithLabel("INTERFACE", UImodeSelection);
	...
	screensaver_row.addElement(std::make_shared<TextComponent>(mWindow, "AJUSTES DO PROTETOR DE TELA", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	s->addWithLabel("TRANSIÇÃO DO CARROSSEL", move_carousel);
	...
	// GameList view style
	auto gamelist_style = std::make_shared< OptionListComponent<std::string> >(mWindow, "MODO DE VISUALIZAÇÃO DA LISTA DE JOGOS", false);
	std::vector<std::string> styles;
	styles.push_back("AUTOMATICO");
	styles.push_back("BASICO");
	styles.push_back("DETALHADO");
	...
	s->addWithLabel("MODO DE VISUALIZAÇÃO DA LISTA DE JOGOS", gamelist_style);
...
{
	auto s = new GuiSettings(mWindow, "OUTROS AJUSTES");

	// maximum vram
	auto max_vram = std::make_shared<SliderComponent>(mWindow, 0.f, 1000.f, 10.f, "Mb");
	max_vram->setValue((float)(Settings::getInstance()->getInt("MaxVRAM")));
	s->addWithLabel("LIMITE DE MEMORIA VRAM", max_vram);
	s->addSaveFunc([max_vram] { Settings::getInstance()->setInt("MaxVRAM", (int)Math::round(max_vram->getValue())); });

	// power saver
	auto power_saver = std::make_shared< OptionListComponent<std::string> >(mWindow, "MODO DE ECONOMIA DE ENERGIA", false);
	...
	s->addWithLabel("MODO DE ECONOMIA DE ENERGIA", power_saver);
	...
	s->addWithLabel("SALVAR METADADOS AO SAIR", save_gamelists);
	s->addSaveFunc([save_gamelists] { Settings::getInstance()->setBool("SaveGamelistsOnExit", save_gamelists->getState()); });

	auto parse_gamelists = std::make_shared<SwitchComponent>(mWindow);
	parse_gamelists->setState(Settings::getInstance()->getBool("ParseGamelistOnly"));
	s->addWithLabel("APENAS PROCESSAR GAMELIST.XML", parse_gamelists);
	...
	s->addWithLabel("MOSTRAR ARQUIVOS OCULTOS", hidden_files);
...
{
	Window* window = mWindow;
	window->pushGui(new GuiMsgBox(window, "VOCE GOSTARIA DE CONFIGURAR CONTROLES-TECLADO?", "Sim",
		[window] {
...
void GuiMenu::openQuitMenu()
{
	...
	if (UIModeController::getInstance()->isUIModeFull())
	{
        ...
        row.makeAcceptInputHandler([window] {
                window->pushGui(new GuiMsgBox(window, "REALMENTE SAIR?", "Sim",
                        [] {
                        SDL_Event ev;
                        ev.type = SDL_QUIT;
                        SDL_PushEvent(&ev);
        ...
        row.addElement(std::make_shared<TextComponent>(window, "SAIR PARA O TERMINAL (requer teclado)", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
        s->addRow(row);
        ...
        row.elements.clear();
        row.makeAcceptInputHandler([window] {
                window->pushGui(new GuiMsgBox(window, "REALMENTE DESLIGAR?", "Sim",
                        [] {
                        if (quitES("/tmp/es-shutdown") != 0)
                                LOG(LogWarning) << "Shutdown terminated with non-zero result!";
                }, "Não", nullptr));
        });
        row.addElement(std::make_shared<TextComponent>(window, "DESLIGAR SISTEMA", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
        s->addRow(row);
                }
        ...
        row.elements.clear();
                row.makeAcceptInputHandler([window] {
                        window->pushGui(new GuiMsgBox(window, "REALMENTE REINICIAR?", "Sim",
                                [] {
...
void GuiMenu::openScreensaverOptions() {
	mWindow->pushGui(new GuiGeneralScreensaverOptions(mWindow, "AJUSTES DO PROTETOR DE TELA"));
}
