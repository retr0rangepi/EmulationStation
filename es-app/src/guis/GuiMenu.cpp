...
GuiMenu::GuiMenu(Window* window) : GuiComponent(window), mMenu(window, "ROPi 4.2 FULL"), mVersion(window)
{
	...
	if (!(UIModeController::getInstance()->isUIModeKid() && Settings::getInstance()->getBool("hideQuitMenuOnKidUI")))
        ...
        if (isFullUI) addEntry("ΕΠΙΦΑΝΕΙΑ ΕΡΓΑΣΙΑΣ", 0x777777FF, true, [this] { Window* window = mWindow;
                window->pushGui(new GuiMsgBox(window, "ΕΙΣΤΕ ΣΙΓΟΥΡΟΙ ΟΤΙ ΘΕΛΕΤΕ ΝΑ ΜΕΤΑΒΕΙΤΕ ΣΤΗΝ ΕΠΙΦΑΝΕΙΑ ΕΡΓΑΣΙΑΣ", "ΝΑΙ", [window]
	...
	if (isFullUI)
        ...
        if (isFullUI)
		addEntry("ΡΥΘΜΙΣΕΙΣ ΗΧΟΥ", 0x777777FF, true, [this] { openSoundSettings(); });

	if (isFullUI)
		addEntry("ΡΥΘΜΙΣΕΙΣ ΔΙΕΠΑΦΗΣ", 0x777777FF, true, [this] { openUISettings(); });

	if (isFullUI)
		addEntry("ΡΥΘΜΙΣΕΙΣ ΣΥΛΛΟΓΗΣ ΠΑΙΧΝΙΔΙΩΝ", 0x777777FF, true, [this] { openCollectionSystemSettings(); });

        if (isFullUI) addEntry("SLEEP MODE", 0x777777FF, true, [this] { Window* window = mWindow;
                window->pushGui(new GuiMsgBox(window, "REALLY SLEEP?", "ΝΑΙ", [window]
                        { system("/home/pi/RetrOrangePi/Power_Button/Sleep_mode.sh 2> /dev/null"); }, "ΟΧΙ", nullptr) ); });

	if (isFullUI)
		addEntry("ΑΛΛΕΣ ΡΥΘΜΙΣΕΙΣ", 0x777777FF, true, [this] { openOtherSettings(); });
...
void GuiMenu::openScraperSettings()
{
	...
	// scrape from
	auto scraper_list = std::make_shared< OptionListComponent< std::string > >(mWindow, "ΜΕΤΑΦΟΡΤΩΣΗ ΑΞΙΟΛΟΓΗΣΕΩΝ ΑΠΟ - ΔΙΑΛΕΧΤΕ SERVER", false);
	std::vector<std::string> scrapers = getScraperList();
	for(auto it = scrapers.cbegin(); it != scrapers.cend(); it++)
		scraper_list->add(*it, *it, *it == Settings::getInstance()->getString("Scraper"));

	s->addWithLabel("ΜΕΤΑΦΟΡΤΩΣΗ ΑΞΙΟΛΟΓΗΣΕΩΝ ΑΠΟ - ΔΙΑΛΕΧΤΕ SERVER", scraper_list);
	s->addSaveFunc([scraper_list] { Settings::getInstance()->setString("Scraper", scraper_list->getSelected()); });

	// scrape ratings
	auto scrape_ratings = std::make_shared<SwitchComponent>(mWindow);
	scrape_ratings->setState(Settings::getInstance()->getBool("ScrapeRatings"));
	s->addWithLabel("ΜΕΤΑΦΟΡΤΩΣΗ ΑΞΙΟΛΟΓΗΣΕΩΝ ΠΑΙΧΝΙΔΙΩΝ", scrape_ratings);
	...
	auto scrape_now = std::make_shared<TextComponent>(mWindow, "ΜΕΤΑΦΟΡΤΩΣΗ ΑΞΙΟΛΟΓΗΣΕΩΝ ΤΩΡΑ", Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
...
{
	auto s = new GuiSettings(mWindow, "ΡΥΘΜΙΣΕΙΣ ΗΧΟΥ");

	// volume
	auto volume = std::make_shared<SliderComponent>(mWindow, 0.f, 100.f, 1.f, "%");
	volume->setValue((float)VolumeControl::getInstance()->getVolume());
	s->addWithLabel("ΕΝΤΑΣΗ ΣΥΣΤΗΜΑΤΟΣ", volume);
	...
	if (UIModeController::getInstance()->isUIModeFull())
	{
		...
		s->addWithLabel("ΕΝΕΡΓΟΠΟΙΗΣΗ ΗΧΩΝ ΠΛΟΗΓΗΣΗΣ", sounds_enabled);
		...
		s->addWithLabel("ΕΝΕΡΓΟΠΟΙΗΣΗ ΗΧΩΝ ΒΙΝΤΕΟ", video_audio);
...
{
	auto s = new GuiSettings(mWindow, "ΡΥΘΜΙΣΕΙΣ ΔΙΕΠΑΦΗΣ");
	...
	s->addWithLabel("ΓΡΗΓΟΡΗ ΕΠΙΛΟΓΗ ΣΥΣΤΗΜΑΤΟΣ", quick_sys_select);
	s->addSaveFunc([quick_sys_select] { Settings::getInstance()->setBool("QuickSystemSelect", quick_sys_select->getState()); });

	// carousel transition option
	auto move_carousel = std::make_shared<SwitchComponent>(mWindow);
	move_carousel->setState(Settings::getInstance()->getBool("MoveCarousel"));
	s->addWithLabel("ΜΕΤΑΒΑΣΗ ΟΘΟΝΗΣ CAROUSEL", move_carousel);
	...
	// transition style
	auto transition_style = std::make_shared< OptionListComponent<std::string> >(mWindow, "ΣΤΥΛ ΜΕΤΑΒΑΣΗΣ ΟΘΟΝΗΣ", false);
	std::vector<std::string> transitions;
	...
	s->addWithLabel("ΣΤΥΛ ΜΕΤΑΒΑΣΗΣ ΟΘΟΝΗΣ", transition_style);
	...
	if(!themeSets.empty())
	{
		...
		auto theme_set = std::make_shared< OptionListComponent<std::string> >(mWindow, "ΕΠΙΛΟΓΗ ΣΤΥΛ ΣΒΗΣΙΜΑΤΟΣ", false);
		for(auto it = themeSets.cbegin(); it != themeSets.cend(); it++)
			theme_set->add(it->first, it->first, it == selectedSet);
		s->addWithLabel("ΕΠΙΛΟΓΗ ΣΤΥΛ ΣΒΗΣΙΜΑΤΟΣ", theme_set);
	...
	// GameList view style
	auto gamelist_style = std::make_shared< OptionListComponent<std::string> >(mWindow, "ΣΤΥΛ ΠΡΟΒΟΛΗΣ ΛΙΣΤΑΣ ΠΑΙΧΝΙΔΙΩΝ", false);
	std::vector<std::string> styles;
	styles.push_back("ΑΥΤΟΜΑΤΟ");
	styles.push_back("ΒΑΣΙΚΟ");
	styles.push_back("ΛΕΠΤΟΜΕΡΕΣ");
	styles.push_back("ΒΙΝΤΕΟ");
	...
	s->addWithLabel("ΣΤΥΛ ΠΡΟΒΟΛΗΣ ΛΙΣΤΑΣ ΠΑΙΧΝΙΔΙΩΝ", gamelist_style);
	...
	s->addWithLabel("ΒΟΗΘΕΙΑ ΟΘΟΝΗΣ", show_help);
...
{
	auto s = new GuiSettings(mWindow, "ΑΛΛΕΣ ΡΥΘΜΙΣΕΙΣ");

	// maximum vram
	auto max_vram = std::make_shared<SliderComponent>(mWindow, 0.f, 1000.f, 10.f, "Mb");
	max_vram->setValue((float)(Settings::getInstance()->getInt("MaxVRAM")));
	s->addWithLabel("ΟΡΙΟ VRAM", max_vram);
	s->addSaveFunc([max_vram] { Settings::getInstance()->setInt("MaxVRAM", (int)Math::round(max_vram->getValue())); });

	// power saver
	auto power_saver = std::make_shared< OptionListComponent<std::string> >(mWindow, "ΛΕΙΤΟΥΡΓΙΑ ΕΞΟΙΚΟΝΟΜΙΣΗΣ ΕΝΕΡΓΕΙΑΣ", false);
	...
	s->addWithLabel("ΛΕΙΤΟΥΡΓΙΑ ΕΞΟΙΚΟΝΟΜΙΣΗΣ ΕΝΕΡΓΕΙΑΣ", power_saver);
	...
	s->addWithLabel("ΣΩΣΤΕ ΤΑ ΜΕΤΑΔΕΔΟΜΕΝΑ ΣΤΗΝ ΕΞΟΔΟ", save_gamelists);
	s->addSaveFunc([save_gamelists] { Settings::getInstance()->setBool("SaveGamelistsOnExit", save_gamelists->getState()); });

	auto parse_gamelists = std::make_shared<SwitchComponent>(mWindow);
	parse_gamelists->setState(Settings::getInstance()->getBool("ParseGamelistOnly"));
	s->addWithLabel("ΑΝΑΛΥΣΗ ΛΙΣΤΩΝ ΠΑΙΧΝΙΔΙΩΝ ΜΟΝΟ", parse_gamelists);
	...
	s->addWithLabel("ΕΜΦΑΝΙΣΗ ΚΡΥΦΩΝ ΑΡΧΕΙΩΝ", hidden_files);
	...
	s->addWithLabel("ΕΜΦΑΝΙΣΗ ΑΝΑΝΕΩΣΕΩΝ ΟΘΟΝΗΣ", framerate);
...
{
	Window* window = mWindow;
	window->pushGui(new GuiMsgBox(window, "ΕΙΣΤΕ ΣΙΓΟΥΡΟΙ ΠΩΣ ΘΕΛΕΝΕ ΝΑ ΕΠΕΞΕΡΓΑΣΤΕΙΤΕ ΤΙΣ ΣΥΣΚΕΥΕΣ ΕΙΣΟΔΟΥ;", "ΝΑΙ",
		[window] {
...
void GuiMenu::openQuitMenu()
{
	...
	if (UIModeController::getInstance()->isUIModeFull())
	{
        ...
        row.makeAcceptInputHandler([window] {
                window->pushGui(new GuiMsgBox(window, "REALLY QUIT?", "ΝΑΙ",
                        [] {
                        SDL_Event ev;
                        ev.type = SDL_QUIT;
                        SDL_PushEvent(&ev);
        ...
        row.elements.clear();
        row.makeAcceptInputHandler([window] {
                window->pushGui(new GuiMsgBox(window, "REALLY SHUTDOWN?", "ΝΑΙ",
                        [] {
        ...
        row.elements.clear();
                row.makeAcceptInputHandler([window] {
                        window->pushGui(new GuiMsgBox(window, "REALLY RESTART?", "ΝΑΙ",
                                [] {
