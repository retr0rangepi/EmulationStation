...
GuiMenu::GuiMenu(Window* window) : GuiComponent(window), mMenu(window, "ROPi 4.2 FULL"), mVersion(window)
{
	...
	if (!(UIModeController::getInstance()->isUIModeKid() && Settings::getInstance()->getBool("hideQuitMenuOnKidUI")))
        ...
        if (isFullUI) addEntry("ESCRITORIO -  MODO PC", 0x777777FF, true, [this] { Window* window = mWindow;
                window->pushGui(new GuiMsgBox(window, "¿SEGURO DE QUE QUIERES EJECUTAR EL ESCRITORIO?", "SI", [window]
                        { system("startx 2> /dev/null"); }, "NO", nullptr) ); });

	if (isFullUI)
		addEntry("CONFIGURAR CONTROL", 0x777777FF, true, [this] { openConfigInput(); });

	if (isFullUI)
		addEntry("CARATULAS", 0x777777FF, true, [this] { openScraperSettings(); });
	...
	if (isFullUI)
		addEntry("AJUSTES BIBLIOTECA DE JUEGOS", 0x777777FF, true, [this] { openCollectionSystemSettings(); });
...
{
	auto s = new GuiSettings(mWindow, "CARATULAS");

	// scrape from
	auto scraper_list = std::make_shared< OptionListComponent< std::string > >(mWindow, "DESCARGAR DE", false);
	std::vector<std::string> scrapers = getScraperList();
	for(auto it = scrapers.cbegin(); it != scrapers.cend(); it++)
		scraper_list->add(*it, *it, *it == Settings::getInstance()->getString("Scraper"));

	s->addWithLabel("DESCARGAR DE", scraper_list);
	s->addSaveFunc([scraper_list] { Settings::getInstance()->setString("Scraper", scraper_list->getSelected()); });

	// scrape ratings
	auto scrape_ratings = std::make_shared<SwitchComponent>(mWindow);
	scrape_ratings->setState(Settings::getInstance()->getBool("ScrapeRatings"));
	s->addWithLabel("PUNTAJES", scrape_ratings);
	...
	auto scrape_now = std::make_shared<TextComponent>(mWindow, "DESCARGAR ", Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
...
void GuiMenu::openSoundSettings()
{
	...
	if (UIModeController::getInstance()->isUIModeFull())
	{
		...
		s->addWithLabel("ACTIVAR SONIDO DE NAVEGACION", sounds_enabled);
		...
		s->addWithLabel("ACTIVAR AUDIO VIDEO", video_audio);
...
void GuiMenu::openUISettings()
{
	...
	s->addWithLabel("TRANSICION DE CARRUSEL", move_carousel);
	...
	// transition style
	auto transition_style = std::make_shared< OptionListComponent<std::string> >(mWindow, "ESTILO DE TRANSICION", false);
	std::vector<std::string> transitions;
	...
	s->addWithLabel("ESTILO DE TRANSICION", transition_style);
	...
	if(!themeSets.empty())
	{
		...
		auto theme_set = std::make_shared< OptionListComponent<std::string> >(mWindow, "AJUSTES DE TEMA", false);
		for(auto it = themeSets.cbegin(); it != themeSets.cend(); it++)
			theme_set->add(it->first, it->first, it == selectedSet);
		s->addWithLabel("AJUSTES DE TEMA", theme_set);
...
void GuiMenu::openOtherSettings()
{
	...
	s->addWithLabel("ANALIZAR SOLO GAMELISTS", parse_gamelists);
	...
	s->addWithLabel("MOSTRAR FOTOGRAMAS", framerate);
...
{
	Window* window = mWindow;
	window->pushGui(new GuiMsgBox(window, "¿SEGURO QUE QUIERES CONFIGURAR EL CONTROL?", "SI",
		[window] {
...
void GuiMenu::openQuitMenu()
{
	...
	if (UIModeController::getInstance()->isUIModeFull())
	{
        ...
        row.makeAcceptInputHandler([window] {
                window->pushGui(new GuiMsgBox(window, "¿REALMENTE QUIERES SALIR DE EMULATIONSTATION?", "SI",
                        [] {
                        SDL_Event ev;
                        ev.type = SDL_QUIT;
                        SDL_PushEvent(&ev);
        ...
        row.elements.clear();
        row.makeAcceptInputHandler([window] {
                window->pushGui(new GuiMsgBox(window, "¿QUIERES APAGAR?", "SI",
                        [] {
                        if (quitES("/tmp/es-shutdown") != 0)
                                LOG(LogWarning) << "Shutdown terminated with non-zero result!";
                }, "NO", nullptr));
        });
        row.addElement(std::make_shared<TextComponent>(window, "APAGAR", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
        s->addRow(row);
                }
        ...
        row.elements.clear();
                row.makeAcceptInputHandler([window] {
                        window->pushGui(new GuiMsgBox(window, "¿EN SERIO QUIERES REINICIAR?", "SI",
                                [] {
