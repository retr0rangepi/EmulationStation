...
GuiMenu::GuiMenu(Window* window) : GuiComponent(window), mMenu(window, "ROPi 4.2 FULL"), mVersion(window)
{
	...
	if (!(UIModeController::getInstance()->isUIModeKid() && Settings::getInstance()->getBool("hideQuitMenuOnKidUI")))
        ...
        if (isFullUI) addEntry("桌面", 0x777777FF, true, [this] { Window* window = mWindow;
                window->pushGui(new GuiMsgBox(window, "是否确定进入桌面？", "是", [window]
                        { system("startx 2> /dev/null"); }, "否", nullptr) ); });

	if (isFullUI)
		addEntry("设置手柄", 0x777777FF, true, [this] { openConfigInput(); });

	if (isFullUI)
		addEntry("SCRAPER", 0x777777FF, true, [this] { openScraperSettings(); });

        if (isFullUI)
		addEntry("声音设置", 0x777777FF, true, [this] { openSoundSettings(); });
...
void GuiMenu::openScraperSettings()
{
	...
	// scrape from
	auto scraper_list = std::make_shared< OptionListComponent< std::string > >(mWindow, "SCRAPE从", false);
	std::vector<std::string> scrapers = getScraperList();
	for(auto it = scrapers.cbegin(); it != scrapers.cend(); it++)
		scraper_list->add(*it, *it, *it == Settings::getInstance()->getString("Scraper"));

	s->addWithLabel("SCRAPE从", scraper_list);
	s->addSaveFunc([scraper_list] { Settings::getInstance()->setString("Scraper", scraper_list->getSelected()); });

	// scrape ratings
	auto scrape_ratings = std::make_shared<SwitchComponent>(mWindow);
	scrape_ratings->setState(Settings::getInstance()->getBool("ScrapeRatings"));
	s->addWithLabel("SCRAPE评分", scrape_ratings);
...
void GuiMenu::openSoundSettings()
{
	...
	if (UIModeController::getInstance()->isUIModeFull())
	{
		...
		s->addWithLabel("开启导航声音", sounds_enabled);
		...
		s->addWithLabel("开启视频声音", video_audio);
...
void GuiMenu::openUISettings()
{
	...
	s->addWithLabel("快速游戏主机选择", quick_sys_select);
	s->addSaveFunc([quick_sys_select] { Settings::getInstance()->setBool("QuickSystemSelect", quick_sys_select->getState()); });

	// carousel transition option
	auto move_carousel = std::make_shared<SwitchComponent>(mWindow);
	move_carousel->setState(Settings::getInstance()->getBool("MoveCarousel"));
	s->addWithLabel("旋转过渡", move_carousel);
	...
	// GameList view style
	auto gamelist_style = std::make_shared< OptionListComponent<std::string> >(mWindow, "游戏列表预览式样", false);
	...
	s->addWithLabel("游戏列表预览式样", gamelist_style);
...
void GuiMenu::openOtherSettings()
{
	...
	// power saver
	auto power_saver = std::make_shared< OptionListComponent<std::string> >(mWindow, "省电模式", false);
	...
	s->addWithLabel("省电模式", power_saver);
	...
	s->addWithLabel("只解析游戏列表", parse_gamelists);
...
{
	Window* window = mWindow;
	window->pushGui(new GuiMsgBox(window, "是否确定执行输入设置？", "是",
		[window] {
...
void GuiMenu::openQuitMenu()
{
	...
	if (UIModeController::getInstance()->isUIModeFull())
	{
        ...
        row.makeAcceptInputHandler([window] {
                window->pushGui(new GuiMsgBox(window, "确定退出？", "是",
                        [] {
                        SDL_Event ev;
                        ev.type = SDL_QUIT;
                        SDL_PushEvent(&ev);
        ...
        row.addElement(std::make_shared<TextComponent>(window, "退出模拟器", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
        s->addRow(row);
        ...
        row.elements.clear();
        row.makeAcceptInputHandler([window] {
                window->pushGui(new GuiMsgBox(window, "确定关机？", "是",
                        [] {
        ...
        row.elements.clear();
                row.makeAcceptInputHandler([window] {
                        window->pushGui(new GuiMsgBox(window, "确定重启？", "是",
                                [] {
                                if(quitES("/tmp/es-restart") != 0)
                                        LOG(LogWarning) << "Restart terminated with non-zero result!";
                        }, "否", nullptr));
                });
                row.addElement(std::make_shared<TextComponent>(window, "重启模拟器", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
                s->addRow(row);
