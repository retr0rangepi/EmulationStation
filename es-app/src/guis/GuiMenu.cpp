...
GuiMenu::GuiMenu(Window* window) : GuiComponent(window), mMenu(window, "ROPi 4.2 FULL"), mVersion(window)
{
	...
	if (!(UIModeController::getInstance()->isUIModeKid() && Settings::getInstance()->getBool("hideQuitMenuOnKidUI")))
		addEntry("BEENDEN", 0x777777FF, true, [this] {openQuitMenu(); });

        if (isFullUI) addEntry("DESKTOP", 0x777777FF, true, [this] { Window* window = mWindow;
                window->pushGui(new GuiMsgBox(window, "JETZT ZUM DESKTOP WECHSELN?", "JA", [window]
                        { system("startx 2> /dev/null"); }, "NEIN", nullptr) ); });

	if (isFullUI)
		addEntry("EINGABE-EINSTELLUNGEN", 0x777777FF, true, [this] { openConfigInput(); });

	if (isFullUI)
		addEntry("SCRAPER", 0x777777FF, true, [this] { openScraperSettings(); });

        if (isFullUI)
		addEntry("SOUND-EINSTELLUNGEN", 0x777777FF, true, [this] { openSoundSettings(); });

	if (isFullUI)
		addEntry("OBERFLÄCHEN-EINSTELLUNGEN", 0x777777FF, true, [this] { openUISettings(); });

	if (isFullUI)
		addEntry("EINSTELLUNG SPIELE-SAMMLUNGEN", 0x777777FF, true, [this] { openCollectionSystemSettings(); });

        if (isFullUI) addEntry("SLEEP MODE", 0x777777FF, true, [this] { Window* window = mWindow;
                window->pushGui(new GuiMsgBox(window, "REALLY SLEEP?", "JA", [window]
                        { system("/home/pi/RetrOrangePi/Power_Button/Sleep_mode.sh 2> /dev/null"); }, "NEIN", nullptr) ); });

	if (isFullUI)
		addEntry("ANDERE EINSTELLUNGEN", 0x777777FF, true, [this] { openOtherSettings(); });
...
void GuiMenu::openScraperSettings()
{
	...
	// scrape from
	auto scraper_list = std::make_shared< OptionListComponent< std::string > >(mWindow, "SCRAPEN MIT", false);
	std::vector<std::string> scrapers = getScraperList();
	for(auto it = scrapers.cbegin(); it != scrapers.cend(); it++)
		scraper_list->add(*it, *it, *it == Settings::getInstance()->getString("Scraper"));

	s->addWithLabel("SCRAPEN MIT", scraper_list);
	s->addSaveFunc([scraper_list] { Settings::getInstance()->setString("Scraper", scraper_list->getSelected()); });

	// scrape ratings
	auto scrape_ratings = std::make_shared<SwitchComponent>(mWindow);
	scrape_ratings->setState(Settings::getInstance()->getBool("ScrapeRatings"));
	s->addWithLabel("SCRAPE BEWERTUNGEN", scrape_ratings);
	...
	auto scrape_now = std::make_shared<TextComponent>(mWindow, "BEGINNE SCRAPEN", Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
...
{
	auto s = new GuiSettings(mWindow, "SOUND-EINSTELLUNGEN");

	// volume
	auto volume = std::make_shared<SliderComponent>(mWindow, 0.f, 100.f, 1.f, "%");
	volume->setValue((float)VolumeControl::getInstance()->getVolume());
	s->addWithLabel("SYSTEMLAUTSTÄRKE", volume);
	...
	if (UIModeController::getInstance()->isUIModeFull())
	{
		...
		s->addWithLabel("NAVIGATIONSTÖNE", sounds_enabled);
		...
		s->addWithLabel("VIDEO- UND SOUNDAUSGABE", video_audio);
...
{
	auto s = new GuiSettings(mWindow, "OBERFLÄCHEN-EINSTELLUNGEN");
	...
	s->addWithLabel("SCHNELLE SYSTEMAUSWAHL", quick_sys_select);
	s->addSaveFunc([quick_sys_select] { Settings::getInstance()->setBool("QuickSystemSelect", quick_sys_select->getState()); });

	// carousel transition option
	auto move_carousel = std::make_shared<SwitchComponent>(mWindow);
	move_carousel->setState(Settings::getInstance()->getBool("MoveCarousel"));
	s->addWithLabel("KARUSSELLÜBERGÄNGE", move_carousel);
	...
	// transition style
	auto transition_style = std::make_shared< OptionListComponent<std::string> >(mWindow, "ÜBERGANGSSTIL", false);
	std::vector<std::string> transitions;
	...
	s->addWithLabel("ÜBERGANGSSTIL", transition_style);
	...
	if(!themeSets.empty())
	{
		...
		auto theme_set = std::make_shared< OptionListComponent<std::string> >(mWindow, "THEME AUSWÄHLEN", false);
		for(auto it = themeSets.cbegin(); it != themeSets.cend(); it++)
			theme_set->add(it->first, it->first, it == selectedSet);
		s->addWithLabel("THEME AUSWÄHLEN", theme_set);
	...
	// GameList view style
	auto gamelist_style = std::make_shared< OptionListComponent<std::string> >(mWindow, "SPIELLISTE ANZEIGEART", false);
	std::vector<std::string> styles;
	styles.push_back("AUTOMATISCH");
	styles.push_back("BASIS");
	styles.push_back("DETAILLIERT");
	...
	s->addWithLabel("SPIELLISTE ANZEIGEART", gamelist_style);
	...
	s->addWithLabel("BILDSCHIRM HILFE", show_help);
...
{
	auto s = new GuiSettings(mWindow, "ANDERE EINSTELLUNGEN");
	...
	// power saver
	auto power_saver = std::make_shared< OptionListComponent<std::string> >(mWindow, "STROMSPAR-EINSTELLUNGEN", false);
	...
	s->addWithLabel("STROMSPAR-EINSTELLUNGEN", power_saver);
	...
	s->addWithLabel("METADATEN BEIM VERLASSEN SPEICHERN", save_gamelists);
	s->addSaveFunc([save_gamelists] { Settings::getInstance()->setBool("SaveGamelistsOnExit", save_gamelists->getState()); });

	auto parse_gamelists = std::make_shared<SwitchComponent>(mWindow);
	parse_gamelists->setState(Settings::getInstance()->getBool("ParseGamelistOnly"));
	s->addWithLabel("NUR SPIELE-LISTEN VERARBEITEN", parse_gamelists);
	...
	s->addWithLabel("VERSTECKTE DATEIEN ANZEIGEN", hidden_files);
	...
	s->addWithLabel("FRAMERATE ANZEIGEN", framerate);
...
{
	Window* window = mWindow;
	window->pushGui(new GuiMsgBox(window, "WIRKLICH DIE EINGABE-EINSTELLUNGEN ÄNDERN?", "JA",
		[window] {
...
{
	auto s = new GuiSettings(mWindow, "BEENDEN");
	...
	if (UIModeController::getInstance()->isUIModeFull())
	{
        ...
        row.makeAcceptInputHandler([window] {
                window->pushGui(new GuiMsgBox(window, "WIRKLICH BEENDEN?", "JA",
                        [] {
                        SDL_Event ev;
                        ev.type = SDL_QUIT;
                        SDL_PushEvent(&ev);
        ...
        row.addElement(std::make_shared<TextComponent>(window, "BEENDE EMULATIONSTATION", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
        s->addRow(row);
        ...
        row.elements.clear();
        row.makeAcceptInputHandler([window] {
                window->pushGui(new GuiMsgBox(window, "WIRKLICH HERUNTERFAHREN?", "JA",
                        [] {
                        if (quitES("/tmp/es-shutdown") != 0)
                                LOG(LogWarning) << "Shutdown terminated with non-zero result!";
                }, "NEIN", nullptr));
        });
        row.addElement(std::make_shared<TextComponent>(window, "SYSTEM HERUNTERFAHREN", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
        s->addRow(row);
                }
        ...
        row.elements.clear();
                row.makeAcceptInputHandler([window] {
                        window->pushGui(new GuiMsgBox(window, "WIRKLICH NEU STARTEN?", "JA",
                                [] {
                                if(quitES("/tmp/es-restart") != 0)
                                        LOG(LogWarning) << "Restart terminated with non-zero result!";
                        }, "NEIN", nullptr));
                });
                row.addElement(std::make_shared<TextComponent>(window, "NEUSTART EMULATIONSTATION", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
                s->addRow(row);
        ...
        row.addElement(std::make_shared<TextComponent>(window, "SYSTEM-NEUSTART", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
        s->addRow(row);
	mWindow->pushGui(s);
}
