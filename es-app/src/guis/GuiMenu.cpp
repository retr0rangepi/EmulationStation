...
GuiMenu::GuiMenu(Window* window) : GuiComponent(window), mMenu(window, "ROPi 4.2 FULL"), mVersion(window)
{
	...
	if (!(UIModeController::getInstance()->isUIModeKid() && Settings::getInstance()->getBool("hideQuitMenuOnKidUI")))
		addEntry("ESCI", 0x777777FF, true, [this] {openQuitMenu(); });

        if (isFullUI) addEntry("DESKTOP", 0x777777FF, true, [this] { Window* window = mWindow;
                window->pushGui(new GuiMsgBox(window, "SEI SICURO DI VOLER AVVIARE DESKTOP?", "SI", [window]
                        { system("startx 2> /dev/null"); }, "NO", nullptr) ); });

	if (isFullUI)
		addEntry("CONFIGURA INPUT", 0x777777FF, true, [this] { openConfigInput(); });

	if (isFullUI)
		addEntry("SCARICA INFO GIOCHI", 0x777777FF, true, [this] { openScraperSettings(); });

        if (isFullUI)
		addEntry("IMPOSTAZIONI SUONO", 0x777777FF, true, [this] { openSoundSettings(); });

	if (isFullUI)
		addEntry("IMPOSTAZIONI GRAFICHE", 0x777777FF, true, [this] { openUISettings(); });

	if (isFullUI)
		addEntry("IMPOSTAZIONI COLLEZIONE GIOCHI", 0x777777FF, true, [this] { openCollectionSystemSettings(); });
	...
	if (isFullUI)
		addEntry("ALTRI SETTAGGI", 0x777777FF, true, [this] { openOtherSettings(); });
...
{
	auto s = new GuiSettings(mWindow, "SCARICA INFO GIOCHI");

	// scrape from
	auto scraper_list = std::make_shared< OptionListComponent< std::string > >(mWindow, "SCARICA DA...", false);
	std::vector<std::string> scrapers = getScraperList();
	for(auto it = scrapers.cbegin(); it != scrapers.cend(); it++)
		scraper_list->add(*it, *it, *it == Settings::getInstance()->getString("Scraper"));

	s->addWithLabel("SCARICA DA...", scraper_list);
	s->addSaveFunc([scraper_list] { Settings::getInstance()->setString("Scraper", scraper_list->getSelected()); });

	// scrape ratings
	auto scrape_ratings = std::make_shared<SwitchComponent>(mWindow);
	scrape_ratings->setState(Settings::getInstance()->getBool("ScrapeRatings"));
	s->addWithLabel("SCARICA VOTO GIOCO", scrape_ratings);
	...
	auto scrape_now = std::make_shared<TextComponent>(mWindow, "SCARICA ORA", Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
...
{
	auto s = new GuiSettings(mWindow, "IMPOSTAZIONI SUONO");

	// volume
	auto volume = std::make_shared<SliderComponent>(mWindow, 0.f, 100.f, 1.f, "%");
	volume->setValue((float)VolumeControl::getInstance()->getVolume());
	s->addWithLabel("IMPOSTAZIONI VOLUME", volume);
	...
	if (UIModeController::getInstance()->isUIModeFull())
	{
		...
		s->addWithLabel("ABILITA SUONI NAVIGAZIONE", sounds_enabled);
		...
		s->addWithLabel("ABILITA VIDEO AUDIO", video_audio);
...
{
	auto s = new GuiSettings(mWindow, "IMPOSTAZIONI GRAFICHE");
	...
	s->addWithLabel("SELEZIONE VELOCE SISTEMA", quick_sys_select);
	s->addSaveFunc([quick_sys_select] { Settings::getInstance()->setBool("QuickSystemSelect", quick_sys_select->getState()); });

	// carousel transition option
	auto move_carousel = std::make_shared<SwitchComponent>(mWindow);
	move_carousel->setState(Settings::getInstance()->getBool("MoveCarousel"));
	s->addWithLabel("ANIMAZIONE TRANSIZIONE", move_carousel);
	...
	// transition style
	auto transition_style = std::make_shared< OptionListComponent<std::string> >(mWindow, "TIPOLOGIA TRANSIZIONE", false);
	std::vector<std::string> transitions;
	...
	s->addWithLabel("TIPOLOGIA TRANSIZIONE", transition_style);
	...
	if(!themeSets.empty())
	{
		...
		auto theme_set = std::make_shared< OptionListComponent<std::string> >(mWindow, "IMPOSTA TIPO DI DISSOLVENZA", false);
		for(auto it = themeSets.cbegin(); it != themeSets.cend(); it++)
			theme_set->add(it->first, it->first, it == selectedSet);
		s->addWithLabel("IMPOSTA TIPO DI DISSOLVENZA", theme_set);
	...
	// GameList view style
	auto gamelist_style = std::make_shared< OptionListComponent<std::string> >(mWindow, "Stile della lista giochi", false);
	std::vector<std::string> styles;
	styles.push_back("Automatico");
	styles.push_back("Base");
	styles.push_back("Dettagli");
	...
	s->addWithLabel("Stile della lista giochi", gamelist_style);
	...
	s->addWithLabel("Help sullo schermo", show_help);
...
{
	auto s = new GuiSettings(mWindow, "Altri settaggi");

	// maximum vram
	auto max_vram = std::make_shared<SliderComponent>(mWindow, 0.f, 1000.f, 10.f, "Mb");
	max_vram->setValue((float)(Settings::getInstance()->getInt("MaxVRAM")));
	s->addWithLabel("Limite VRam", max_vram);
	s->addSaveFunc([max_vram] { Settings::getInstance()->setInt("MaxVRAM", (int)Math::round(max_vram->getValue())); });

	// power saver
	auto power_saver = std::make_shared< OptionListComponent<std::string> >(mWindow, "Modalità risparmio energetico", false);
	...
	s->addWithLabel("Modalità risparmio energetico", power_saver);
	...
	s->addWithLabel("Salva metadati all'uscita", save_gamelists);
	s->addSaveFunc([save_gamelists] { Settings::getInstance()->setBool("SaveGamelistsOnExit", save_gamelists->getState()); });

	auto parse_gamelists = std::make_shared<SwitchComponent>(mWindow);
	parse_gamelists->setState(Settings::getInstance()->getBool("ParseGamelistOnly"));
	s->addWithLabel("Leggi solo gamelist", parse_gamelists);
	...
	s->addWithLabel("Mostra files nascosti", hidden_files);
	...
	s->addWithLabel("Mostra framerate", framerate);
...
{
	Window* window = mWindow;
	window->pushGui(new GuiMsgBox(window, "SEI SICURO DI VOLER CONFIGURARE L'INPUT", "SI",
		[window] {
...
{
	auto s = new GuiSettings(mWindow, "ESCI");
	...
	if (UIModeController::getInstance()->isUIModeFull())
	{
        ...
        row.makeAcceptInputHandler([window] {
                window->pushGui(new GuiMsgBox(window, "Esci davvero?", "SI",
                        [] {
                        SDL_Event ev;
                        ev.type = SDL_QUIT;
                        SDL_PushEvent(&ev);
        ...
        row.addElement(std::make_shared<TextComponent>(window, "Chiudi EmulationStation", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
        s->addRow(row);
        ...
        row.elements.clear();
        row.makeAcceptInputHandler([window] {
                window->pushGui(new GuiMsgBox(window, "Vuoi davvero spegnere?", "SI",
                        [] {
                        if (quitES("/tmp/es-shutdown") != 0)
                                LOG(LogWarning) << "Shutdown terminated with non-zero result!";
                }, "NO", nullptr));
        });
        row.addElement(std::make_shared<TextComponent>(window, "Arresta il sistema", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
        s->addRow(row);
                }
        ...
        row.elements.clear();
                row.makeAcceptInputHandler([window] {
                        window->pushGui(new GuiMsgBox(window, "VUOI VERAMENTE RIAVVIARE?", "SI",
                                [] {
                                if(quitES("/tmp/es-restart") != 0)
                                        LOG(LogWarning) << "Restart terminated with non-zero result!";
                        }, "NO", nullptr));
                });
                row.addElement(std::make_shared<TextComponent>(window, "RIAVVIA EMULATION STATION", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
                s->addRow(row);
        ...
        row.addElement(std::make_shared<TextComponent>(window, "RIAVVIA IL SISTEMA", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
        s->addRow(row);
	mWindow->pushGui(s);
}
