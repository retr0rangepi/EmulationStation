...
GuiMenu::GuiMenu(Window* window) : GuiComponent(window), mMenu(window, "ROPi 4.2 FULL"), mVersion(window)
{
	...
	if (!(UIModeController::getInstance()->isUIModeKid() && Settings::getInstance()->getBool("hideQuitMenuOnKidUI")))
        ...
        if (isFullUI) addEntry("桌面", 0x777777FF, true, [this] { Window* window = mWindow;
                window->pushGui(new GuiMsgBox(window, "是否確定進入桌面？", "是", [window]
                        { system("startx 2> /dev/null"); }, "否", nullptr) ); });

	if (isFullUI)
		addEntry("設置手柄", 0x777777FF, true, [this] { openConfigInput(); });

	if (isFullUI)
		addEntry("SCRAPER", 0x777777FF, true, [this] { openScraperSettings(); });

        if (isFullUI)
		addEntry("聲音設置", 0x777777FF, true, [this] { openSoundSettings(); });

	if (isFullUI)
		addEntry("介面設置", 0x777777FF, true, [this] { openUISettings(); });

	if (isFullUI)
		addEntry("遊戲收藏設置", 0x777777FF, true, [this] { openCollectionSystemSettings(); });

        if (isFullUI) addEntry("SLEEP MODE", 0x777777FF, true, [this] { Window* window = mWindow;
                window->pushGui(new GuiMsgBox(window, "REALLY SLEEP?", "是", [window]
                        { system("/home/pi/RetrOrangePi/Power_Button/Sleep_mode.sh 2> /dev/null"); }, "否", nullptr) ); });

	if (isFullUI)
		addEntry("其他設置", 0x777777FF, true, [this] { openOtherSettings(); });
...
void GuiMenu::openScraperSettings()
{
	...
	// scrape from
	auto scraper_list = std::make_shared< OptionListComponent< std::string > >(mWindow, "SCRAPE從", false);
	std::vector<std::string> scrapers = getScraperList();
	for(auto it = scrapers.cbegin(); it != scrapers.cend(); it++)
		scraper_list->add(*it, *it, *it == Settings::getInstance()->getString("Scraper"));

	s->addWithLabel("SCRAPE從", scraper_list);
	s->addSaveFunc([scraper_list] { Settings::getInstance()->setString("Scraper", scraper_list->getSelected()); });

	// scrape ratings
	auto scrape_ratings = std::make_shared<SwitchComponent>(mWindow);
	scrape_ratings->setState(Settings::getInstance()->getBool("ScrapeRatings"));
	s->addWithLabel("SCRAPE評分", scrape_ratings);
	...
	auto scrape_now = std::make_shared<TextComponent>(mWindow, "立刻SCRAPE", Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
...
{
	auto s = new GuiSettings(mWindow, "聲音設置");

	// volume
	auto volume = std::make_shared<SliderComponent>(mWindow, 0.f, 100.f, 1.f, "%");
	volume->setValue((float)VolumeControl::getInstance()->getVolume());
	s->addWithLabel("系統音量", volume);
	...
	if (UIModeController::getInstance()->isUIModeFull())
	{
		...
		s->addWithLabel("開啟導航聲音", sounds_enabled);
		...
		s->addWithLabel("開啟視頻聲音", video_audio);
...
{
	auto s = new GuiSettings(mWindow, "介面設置");
	...
	s->addWithLabel("快速遊戲主機選擇", quick_sys_select);
	s->addSaveFunc([quick_sys_select] { Settings::getInstance()->setBool("QuickSystemSelect", quick_sys_select->getState()); });

	// carousel transition option
	auto move_carousel = std::make_shared<SwitchComponent>(mWindow);
	move_carousel->setState(Settings::getInstance()->getBool("MoveCarousel"));
	s->addWithLabel("旋轉過渡", move_carousel);
	...
	// transition style
	auto transition_style = std::make_shared< OptionListComponent<std::string> >(mWindow, "過渡樣式", false);
	std::vector<std::string> transitions;
	...
	s->addWithLabel("過渡樣式", transition_style);
	...
	if(!themeSets.empty())
	{
		...
		auto theme_set = std::make_shared< OptionListComponent<std::string> >(mWindow, "主題設置", false);
		for(auto it = themeSets.cbegin(); it != themeSets.cend(); it++)
			theme_set->add(it->first, it->first, it == selectedSet);
		s->addWithLabel("主題設置", theme_set);
	...
	// GameList view style
	auto gamelist_style = std::make_shared< OptionListComponent<std::string> >(mWindow, "遊戲列表預覽式樣", false);
	...
	s->addWithLabel("遊戲列表預覽式樣", gamelist_style);
...
{
	auto s = new GuiSettings(mWindow, "其他設置");

	// maximum vram
	auto max_vram = std::make_shared<SliderComponent>(mWindow, 0.f, 1000.f, 10.f, "Mb");
	max_vram->setValue((float)(Settings::getInstance()->getInt("MaxVRAM")));
	s->addWithLabel("顯存限制", max_vram);
	s->addSaveFunc([max_vram] { Settings::getInstance()->setInt("MaxVRAM", (int)Math::round(max_vram->getValue())); });

	// power saver
	auto power_saver = std::make_shared< OptionListComponent<std::string> >(mWindow, "省電模式", false);
	...
	s->addWithLabel("省電模式", power_saver);
	...
	s->addWithLabel("退出時保存遊戲數據", save_gamelists);
	s->addSaveFunc([save_gamelists] { Settings::getInstance()->setBool("SaveGamelistsOnExit", save_gamelists->getState()); });

	auto parse_gamelists = std::make_shared<SwitchComponent>(mWindow);
	parse_gamelists->setState(Settings::getInstance()->getBool("ParseGamelistOnly"));
	s->addWithLabel("只解析遊戲列表", parse_gamelists);
	...
	s->addWithLabel("顯示隱藏文件", hidden_files);
...
{
	Window* window = mWindow;
	window->pushGui(new GuiMsgBox(window, "是否確定執行手柄設置？", "是",
		[window] {
