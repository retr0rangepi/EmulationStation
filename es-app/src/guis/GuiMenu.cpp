...
GuiMenu::GuiMenu(Window* window) : GuiComponent(window), mMenu(window, "ROPi 4.2 FULL"), mVersion(window)
{
	...
	if (!(UIModeController::getInstance()->isUIModeKid() && Settings::getInstance()->getBool("hideQuitMenuOnKidUI")))
		addEntry("STOPPEN", 0x777777FF, true, [this] {openQuitMenu(); });

        if (isFullUI) addEntry("DESKTOP", 0x777777FF, true, [this] { Window* window = mWindow;
                window->pushGui(new GuiMsgBox(window, "WEET JE ZEKER DAT JE DE DESKTOP WILT STARTEN?", "JA", [window]
                        { system("startx 2> /dev/null"); }, "NEE", nullptr) ); });

	if (isFullUI)
		addEntry("JOYSTICK INSTELLINGEN", 0x777777FF, true, [this] { openConfigInput(); });

	if (isFullUI)
		addEntry("SCRAPER", 0x777777FF, true, [this] { openScraperSettings(); });

        if (isFullUI)
		addEntry("GELUIDSINSTELLINGEN", 0x777777FF, true, [this] { openSoundSettings(); });

	if (isFullUI)
		addEntry("UI INSTELLINGEN", 0x777777FF, true, [this] { openUISettings(); });

	if (isFullUI)
		addEntry("GAME VERZAMELING INSTELLINGEN", 0x777777FF, true, [this] { openCollectionSystemSettings(); });
        ...
        if (isFullUI) addEntry("SLEEP MODE", 0x777777FF, true, [this] { Window* window = mWindow;
                window->pushGui(new GuiMsgBox(window, "REALLY SLEEP?", "JA", [window]
                        { system("/home/pi/RetrOrangePi/Power_Button/Sleep_mode.sh 2> /dev/null"); }, "NEE", nullptr) ); });

	if (isFullUI)
		addEntry("OVERIGE INSTELLINGEN", 0x777777FF, true, [this] { openOtherSettings(); });
...
void GuiMenu::openScraperSettings()
{
	...
	// scrape from
	auto scraper_list = std::make_shared< OptionListComponent< std::string > >(mWindow, "SCRAPER SCRAPE VAN SCRAPE", false);
	std::vector<std::string> scrapers = getScraperList();
	for(auto it = scrapers.cbegin(); it != scrapers.cend(); it++)
		scraper_list->add(*it, *it, *it == Settings::getInstance()->getString("Scraper"));

	s->addWithLabel("SCRAPER SCRAPE VAN SCRAPE", scraper_list);
	s->addSaveFunc([scraper_list] { Settings::getInstance()->setString("Scraper", scraper_list->getSelected()); });

	// scrape ratings
	auto scrape_ratings = std::make_shared<SwitchComponent>(mWindow);
	scrape_ratings->setState(Settings::getInstance()->getBool("ScrapeRatings"));
	s->addWithLabel("SCRAPE SCORE VAN SCRAPE", scrape_ratings);
...
{
	auto s = new GuiSettings(mWindow, "GELUIDSINSTELLINGEN");

	// volume
	auto volume = std::make_shared<SliderComponent>(mWindow, 0.f, 100.f, 1.f, "%");
	volume->setValue((float)VolumeControl::getInstance()->getVolume());
	s->addWithLabel("SYSTEEM VOLUME", volume);
	...
	if (UIModeController::getInstance()->isUIModeFull())
	{
		...
		s->addWithLabel("SCHAKEL NAVIGATIE GELUIDEN IN", sounds_enabled);
		...
		s->addWithLabel("SCHAKEL VIDEO AUDIO IN", video_audio);
...
{
	auto s = new GuiSettings(mWindow, "UI INSTELLINGEN");
	...
	s->addWithLabel("SNELLE SYSTEEM KEUZE", quick_sys_select);
	s->addSaveFunc([quick_sys_select] { Settings::getInstance()->setBool("QuickSystemSelect", quick_sys_select->getState()); });

	// carousel transition option
	auto move_carousel = std::make_shared<SwitchComponent>(mWindow);
	move_carousel->setState(Settings::getInstance()->getBool("MoveCarousel"));
	s->addWithLabel("KEUZE CARROUSEL VERVORMING", move_carousel);
	...
	// transition style
	auto transition_style = std::make_shared< OptionListComponent<std::string> >(mWindow, "VERVORMING STIJL", false);
	std::vector<std::string> transitions;
	...
	s->addWithLabel("VERVORMING STIJL", transition_style);
	...
	if(!themeSets.empty())
	{
		...
		auto theme_set = std::make_shared< OptionListComponent<std::string> >(mWindow, "VERVAAG", false);
		for(auto it = themeSets.cbegin(); it != themeSets.cend(); it++)
			theme_set->add(it->first, it->first, it == selectedSet);
		s->addWithLabel("VERVAAG", theme_set);
	...
	// GameList view style
	auto gamelist_style = std::make_shared< OptionListComponent<std::string> >(mWindow, "SPELLENLIJST KIJK STIJL", false);
	std::vector<std::string> styles;
	styles.push_back("AUTOMATISCH");
	styles.push_back("BASIS");
	styles.push_back("GEDETAILEERD");
	...
	s->addWithLabel("SPELLENLIJST KIJK STIJL", gamelist_style);
	...
	s->addWithLabel("OP HET SCHERM HULP", show_help);
...
{
	auto s = new GuiSettings(mWindow, "OVERIGE INSTELLINGEN");

	// maximum vram
	auto max_vram = std::make_shared<SliderComponent>(mWindow, 0.f, 1000.f, 10.f, "Mb");
	max_vram->setValue((float)(Settings::getInstance()->getInt("MaxVRAM")));
	s->addWithLabel("VRAM LIMIET", max_vram);
	s->addSaveFunc([max_vram] { Settings::getInstance()->setInt("MaxVRAM", (int)Math::round(max_vram->getValue())); });

	// power saver
	auto power_saver = std::make_shared< OptionListComponent<std::string> >(mWindow, "STROOM BESPARINGSMODES", false);
	...
	s->addWithLabel("STROOM BESPARINGSMODES", power_saver);
	...
	s->addWithLabel("SAVE METADATA BIJ AFSLUITEN", save_gamelists);
	s->addSaveFunc([save_gamelists] { Settings::getInstance()->setBool("SaveGamelistsOnExit", save_gamelists->getState()); });

	auto parse_gamelists = std::make_shared<SwitchComponent>(mWindow);
	parse_gamelists->setState(Settings::getInstance()->getBool("ParseGamelistOnly"));
	s->addWithLabel("VERSTUUR ALLEEN GAME LIJST", parse_gamelists);
	...
	s->addWithLabel("TOON VERBORGEN BESTANDEN", hidden_files);
	...
	s->addWithLabel("TOON FRAMERATE", framerate);
...
{
	Window* window = mWindow;
	window->pushGui(new GuiMsgBox(window, "WEET JE ZEKER DAT JE DE JOYSTICK OPNIEUW WILT INSTELLEN", "JA",
		[window] {
...
{
	auto s = new GuiSettings(mWindow, "STOPPEN");
	...
	if (UIModeController::getInstance()->isUIModeFull())
	{
        ...
        row.makeAcceptInputHandler([window] {
                window->pushGui(new GuiMsgBox(window, "REALLY QUIT?", "JA",
                        [] {
                        SDL_Event ev;
                        ev.type = SDL_QUIT;
                        SDL_PushEvent(&ev);
        ...
        row.addElement(std::make_shared<TextComponent>(window, "EMULATIONSTATION STOPPEN", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
        s->addRow(row);
        ...
        row.elements.clear();
        row.makeAcceptInputHandler([window] {
                window->pushGui(new GuiMsgBox(window, "REALLY SHUTDOWN?", "JA",
                        [] {
                        if (quitES("/tmp/es-shutdown") != 0)
                                LOG(LogWarning) << "Shutdown terminated with non-zero result!";
                }, "NEE", nullptr));
        });
        row.addElement(std::make_shared<TextComponent>(window, "SYSTEEM AFSLUITEN", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
        s->addRow(row);
                }
        ...
        row.elements.clear();
                row.makeAcceptInputHandler([window] {
                        window->pushGui(new GuiMsgBox(window, "REALLY RESTART?", "JA",
                                [] {
                                if(quitES("/tmp/es-restart") != 0)
                                        LOG(LogWarning) << "Restart terminated with non-zero result!";
                        }, "NEE", nullptr));
                });
                row.addElement(std::make_shared<TextComponent>(window, "EMULATIONSTATION OPNIEUW OPSTARTEN-HERSTARTEN", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
                s->addRow(row);
        ...
        row.addElement(std::make_shared<TextComponent>(window, "SYSTEEM OPNIEUW OPSTARTEN-HERSTARTEN", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
        s->addRow(row);
	mWindow->pushGui(s);
}
