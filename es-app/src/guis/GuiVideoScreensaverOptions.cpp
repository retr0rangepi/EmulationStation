...
GuiVideoScreensaverOptions::GuiVideoScreensaverOptions(Window* window, const char* title) : GuiScreensaverOptions(window, title)
{
	...
	addWithLabel("(SANİYE) SONRA VİDEOYU DEĞİŞTİR", swap);
	addSaveFunc([swap] {
	...
	auto ss_info = std::make_shared< OptionListComponent<std::string> >(mWindow, "OYUN BİLGİSİNİ GÖSTER", false);
	...
	addWithLabel("OYUN BİLGİSİNİ EKRAN KORUYUCUDA GÖSTER", ss_info);
	...
	addWithLabel("BAŞLIKLAR İÇİN UYGUN DÜŞÜK ÇÖZÜNÜRLÜĞÜ KULLAN", captions_compatibility);
	...
	addWithLabel("VİDEOYU EKRAN KORUYUCUDA YAY", stretch_screensaver);
