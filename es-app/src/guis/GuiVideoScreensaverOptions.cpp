...
GuiVideoScreensaverOptions::GuiVideoScreensaverOptions(Window* window, const char* title) : GuiScreensaverOptions(window, title)
{
	...
	addWithLabel("SCAMBIA VIDEO DOPO (secs)", swap);
	addSaveFunc([swap] {
	...
	auto ss_info = std::make_shared< OptionListComponent<std::string> >(mWindow, "MOSTRA INFO GIOCO", false);
	...
	addWithLabel("MOSTRA INFO GIOCO SU SCREENSAVER", ss_info);
	...
	addWithLabel("USA RISOLUZIONE COMPATIBILE", captions_compatibility);
	...
	addWithLabel("ESTENDI VIDEO SU SCREENSAVER", stretch_screensaver);
