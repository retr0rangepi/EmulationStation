...
GuiVideoScreensaverOptions::GuiVideoScreensaverOptions(Window* window, const char* title) : GuiScreensaverOptions(window, title)
{
	...
	addWithLabel("CAMBIAR VIDEO TRAS (SEGS)", swap);
	addSaveFunc([swap] {
	...
	auto ss_info = std::make_shared< OptionListComponent<std::string> >(mWindow, "MOSTRAR INFORMACION DEL JUEGO", false);
	...
	addWithLabel("MOSTRAR INFORMACION DEL JUEGO EN SALVAPANTALLAS", ss_info);
	...
	addWithLabel("USAR UNA RECOLUCION BAJA COMPATIBLE CON LOS SUBTITULOS", captions_compatibility);
	...
	addWithLabel("AJUSTAR VIDEO EN EL SALVAPANTALLAS", stretch_screensaver);
