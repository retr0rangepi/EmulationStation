...
GuiVideoScreensaverOptions::GuiVideoScreensaverOptions(Window* window, const char* title) : GuiScreensaverOptions(window, title)
{
	...
	addWithLabel("МЕНЯТЬ ВИДЕО ПОСЛЕ (СЕК)", swap);
	addSaveFunc([swap] {
	...
	auto ss_info = std::make_shared< OptionListComponent<std::string> >(mWindow, "ПОКАЗЫВАТЬ ИНФОРМАЦИЮ ОБ ИГРЕ", false);
	...
	addWithLabel("ПОКАЗЫВАТЬ ИНФОРМАЦИЮ ОБ ИГРЕ В ХРАНИИТЕЛЕ ЭКРАНА", ss_info);
	...
	addWithLabel("СОВМЕСТИМОЕ НИЗКОЕ РАЗРЕШЕНИЕ ДЛЯ ПОДПИСЕЙ", captions_compatibility);
	...
	addWithLabel("РАСТЯГИВАТЬ ВИДЕО ХРАНИТЕЛЯ ЭКРАНА", stretch_screensaver);
