...
GuiVideoScreensaverOptions::GuiVideoScreensaverOptions(Window* window, const char* title) : GuiScreensaverOptions(window, title)
{
	...
	addWithLabel("TROCAR VIDEO APÓS (segs)", swap);
	addSaveFunc([swap] {
	...
	auto ss_info = std::make_shared< OptionListComponent<std::string> >(mWindow, "MOSTRAR INFO DO JOGO", false);
	...
	addWithLabel("MOSTRAR INFO DO JOGO NO PROTETOR DE TELA", ss_info);
	...
	addWithLabel("Usar resoluções compatíveis para as legendas", captions_compatibility);
	...
	addWithLabel("ESTENDER VÍDEO NO SCREENSAVER", stretch_screensaver);
