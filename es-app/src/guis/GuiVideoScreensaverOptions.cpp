...
GuiVideoScreensaverOptions::GuiVideoScreensaverOptions(Window* window, const char* title) : GuiScreensaverOptions(window, title)
{
	...
	addWithLabel("WECHSEL VIDEO NACH (SEK.)", swap);
	addSaveFunc([swap] {
	...
	auto ss_info = std::make_shared< OptionListComponent<std::string> >(mWindow, "ZEIGE SPIEL-INFO", false);
	...
	addWithLabel("ZEIGE SPIEL-INFO IM BILDSCHIRMSCHONER", ss_info);
	...
	addWithLabel("BENUTZE KOMPATIBLE NIEDRIGE AUFLÖSUNG FÜR BESCHRIFTUNG", captions_compatibility);
	...
	addWithLabel("DEHNE VIDEO IM BILDSCHIRMSCHONER", stretch_screensaver);
