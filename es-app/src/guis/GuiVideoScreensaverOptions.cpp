...
GuiVideoScreensaverOptions::GuiVideoScreensaverOptions(Window* window, const char* title) : GuiScreensaverOptions(window, title)
{
	...
	addWithLabel("Cambiar Videos después (Segundos)", swap);
	addSaveFunc([swap] {
	...
	auto ss_info = std::make_shared< OptionListComponent<std::string> >(mWindow, "Mostrar info", false);
	...
	addWithLabel("Mostrar info en el protector de pantalla", ss_info);
	...
	addWithLabel("Utilizar una resolución baja compatible con los subtítulos", captions_compatibility);
	...
	addWithLabel("Ajustar videos al protector de pantalla", stretch_screensaver);
