...
GuiVideoScreensaverOptions::GuiVideoScreensaverOptions(Window* window, const char* title) : GuiScreensaverOptions(window, title)
{
	...
	addWithLabel("ÉCHANGER LA VIDEO APRES (SECS)", swap);
	addSaveFunc([swap] {
	...
	auto ss_info = std::make_shared< OptionListComponent<std::string> >(mWindow, "AFFICHER LES INFOS DU JEU", false);
	...
	addWithLabel("AFFICHER LES INFOS DU JEU SUR L´ÉCRAN DE VEILLE", ss_info);
	...
	addWithLabel("UTILISER UNE FAIBLE RÉSOLUTION COMPATIBLE POUR LES CAPTIONS", captions_compatibility);
	...
	addWithLabel("ÉTENDRE LA VIDEO SUR L´ÉCRAN DE VEILLE", stretch_screensaver);
