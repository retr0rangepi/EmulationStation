...
GuiVideoScreensaverOptions::GuiVideoScreensaverOptions(Window* window, const char* title) : GuiScreensaverOptions(window, title)
{
	...
	addWithLabel("skift video efter(sek.)", swap);
	addSaveFunc([swap] {
	...
	auto ss_info = std::make_shared< OptionListComponent<std::string> >(mWindow, "Vis spil Info", false);
	...
	addWithLabel("Vis spil info på pause skærm", ss_info);
	...
	addWithLabel("Brug kompatibel lav skærmopløsning til Captions", captions_compatibility);
	...
	addWithLabel("Stræk video på pause skærm", stretch_screensaver);
