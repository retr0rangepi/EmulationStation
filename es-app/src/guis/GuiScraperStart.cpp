...
GuiScraperStart::GuiScraperStart(Window* window) : GuiComponent(window),
	mMenu(window, "BUSCAR DATOS")
{
	addChild(&mMenu);

	// add filters (with first one selected)
	mFilters = std::make_shared< OptionListComponent<GameFilterFunc> >(mWindow, "DESCARGAR DE ESOS JUEGOS", false);
	mFilters->add("TODOS LOS JUEGOS", 
		[](SystemData*, FileData*) -> bool { return true; }, false);
	mFilters->add("SOLO CON DATOS INCOMPLETOS", 
		[](SystemData*, FileData* g) -> bool { return g->metadata.get("IMAGEN").empty(); }, true);
	mMenu.addWithLabel("FILTRO", mFilters);

	//add systems (all with a platformid specified selected)
	mSystems = std::make_shared< OptionListComponent<SystemData*> >(mWindow, "CARATULAS DE ESOS SISTEMAS", true);
	...
	mMenu.addWithLabel("USUARIO DECIDE EN CASO DE CONFLICTO", mApproveResults);
...
void GuiScraperStart::start()
{
	...
	{
		mWindow->pushGui(new GuiMsgBox(mWindow,
			"NINGUN JUEGO SE ADAPTA AL CRITERIO"));
	}else{
