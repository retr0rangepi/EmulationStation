...
GuiScraperStart::GuiScraperStart(Window* window) : GuiComponent(window),
	mMenu(window, "Fai lo scraping ora")
{
	addChild(&mMenu);

	// add filters (with first one selected)
	mFilters = std::make_shared< OptionListComponent<GameFilterFunc> >(mWindow, "Scrape di questi giochi", false);
	mFilters->add("Tutti i giochi", 
		[](SystemData*, FileData*) -> bool { return true; }, false);
	mFilters->add("Solo immagini mancanti", 
		[](SystemData*, FileData* g) -> bool { return g->metadata.get("Immagine").empty(); }, true);
	mMenu.addWithLabel("Filtro", mFilters);

	//add systems (all with a platformid specified selected)
	mSystems = std::make_shared< OptionListComponent<SystemData*> >(mWindow, "Scrape di questi sistemi", true);
	for(auto it = SystemData::sSystemVector.cbegin(); it != SystemData::sSystemVector.cend(); it++)
	{
		if(!(*it)->hasPlatformId(PlatformIds::PLATFORM_IGNORE))
			mSystems->add((*it)->getFullName(), *it, !(*it)->getPlatformIds().empty());
	}
	mMenu.addWithLabel("Sistemi", mSystems);

	mApproveResults = std::make_shared<SwitchComponent>(mWindow);
	mApproveResults->setState(true);
	mMenu.addWithLabel("Utente decide i conflitti", mApproveResults);
...
void GuiScraperStart::start()
{
	...
	{
		mWindow->pushGui(new GuiMsgBox(mWindow,
			"Nessun gioco segue quel filtro"));
	}else{
