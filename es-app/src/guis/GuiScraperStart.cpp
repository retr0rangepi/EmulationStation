...
GuiScraperStart::GuiScraperStart(Window* window) : GuiComponent(window),
	mMenu(window, "ЗАГРУЗИТЬ ОПИСАНИЯ СЕЙЧАС")
{
	addChild(&mMenu);

	// add filters (with first one selected)
	mFilters = std::make_shared< OptionListComponent<GameFilterFunc> >(mWindow, "ЗАГРУЗИТЬ ОПИСАНИЯ ЭТИХ ИГР", false);
	mFilters->add("Все игры", 
		[](SystemData*, FileData*) -> bool { return true; }, false);
	mFilters->add("Только отсутствующие изображения", 
		[](SystemData*, FileData* g) -> bool { return g->metadata.get("изображение").empty(); }, true);
	mMenu.addWithLabel("Фильтр", mFilters);

	//add systems (all with a platformid specified selected)
	mSystems = std::make_shared< OptionListComponent<SystemData*> >(mWindow, "ЗАГРУЗИТЬ ОПИСАНИЯ ЭТИХ СИСТЕМ", true);
	for(auto it = SystemData::sSystemVector.cbegin(); it != SystemData::sSystemVector.cend(); it++)
	{
		if(!(*it)->hasPlatformId(PlatformIds::PLATFORM_IGNORE))
			mSystems->add((*it)->getFullName(), *it, !(*it)->getPlatformIds().empty());
	}
	mMenu.addWithLabel("Системы", mSystems);

	mApproveResults = std::make_shared<SwitchComponent>(mWindow);
	mApproveResults->setState(true);
	mMenu.addWithLabel("При конфликте спрашивать", mApproveResults);

	mMenu.addButton("СТАРТ", "start", std::bind(&GuiScraperStart::pressedStart, this));
	...
	for(auto it = sys.cbegin(); it != sys.cend(); it++)
	{
		if((*it)->getPlatformIds().empty())
		{
			mWindow->pushGui(new GuiMsgBox(mWindow, 
				...
				"ДА", std::bind(&GuiScraperStart::start, this), 
				"НЕТ", nullptr));
			return;
		}
	}
...
void GuiScraperStart::start()
{
	...
	{
		mWindow->pushGui(new GuiMsgBox(mWindow,
			"НИ ОДНА ИГРА НЕ СОООТВЕТСТВУЕТ КРИТЕРИЮ."));
	}else{
