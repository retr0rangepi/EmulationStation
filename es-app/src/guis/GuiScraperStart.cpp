...
GuiScraperStart::GuiScraperStart(Window* window) : GuiComponent(window),
	mMenu(window, "BUSCAR METADADOS")
{
	addChild(&mMenu);

	// add filters (with first one selected)
	mFilters = std::make_shared< OptionListComponent<GameFilterFunc> >(mWindow, "BUSCAR ESSES JOGOS", false);
	mFilters->add("TODOS OS JOGOS", 
		[](SystemData*, FileData*) -> bool { return true; }, false);
	mFilters->add("APENAS SEM FIGURAS", 
		[](SystemData*, FileData* g) -> bool { return g->metadata.get("imagem").empty(); }, true);
	mMenu.addWithLabel("FILTRO", mFilters);

	//add systems (all with a platformid specified selected)
	mSystems = std::make_shared< OptionListComponent<SystemData*> >(mWindow, "BUSCAR ESSES SISTEMAS", true);
	...
	mMenu.addWithLabel("USUARIO DECIDE SOBRE CONFLITOS", mApproveResults);
...
void GuiScraperStart::start()
{
	...
	{
		mWindow->pushGui(new GuiMsgBox(mWindow,
			"NENHUM JOGO SE ENQUADRA NO REQUISITO"));
	}else{
