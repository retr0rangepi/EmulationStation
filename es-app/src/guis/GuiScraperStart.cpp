...
GuiScraperStart::GuiScraperStart(Window* window) : GuiComponent(window),
	mMenu(window, "ŞİMDİ ETİKETLE")
{
	addChild(&mMenu);

	// add filters (with first one selected)
	mFilters = std::make_shared< OptionListComponent<GameFilterFunc> >(mWindow, "BU OYUNLARI ETİKETLE", false);
	mFilters->add("Tüm Oyunlar", 
		[](SystemData*, FileData*) -> bool { return true; }, false);
	mFilters->add("Sadece resmi eksik olanlar", 
		[](SystemData*, FileData* g) -> bool { return g->metadata.get("resim").empty(); }, true);
	mMenu.addWithLabel("Filtre", mFilters);

	//add systems (all with a platformid specified selected)
	mSystems = std::make_shared< OptionListComponent<SystemData*> >(mWindow, "BU SİSTEMLERİ ETİKETLE", true);
	for(auto it = SystemData::sSystemVector.cbegin(); it != SystemData::sSystemVector.cend(); it++)
	{
		if(!(*it)->hasPlatformId(PlatformIds::PLATFORM_IGNORE))
			mSystems->add((*it)->getFullName(), *it, !(*it)->getPlatformIds().empty());
	}
	mMenu.addWithLabel("Sistemler", mSystems);

	mApproveResults = std::make_shared<SwitchComponent>(mWindow);
	mApproveResults->setState(true);
	mMenu.addWithLabel("Çakışmalara kullanıcı karar verir", mApproveResults);

	mMenu.addButton("START", "start", std::bind(&GuiScraperStart::pressedStart, this));
	mMenu.addButton("GERİ", "back", [&] { delete this; });
	...
	for(auto it = sys.cbegin(); it != sys.cend(); it++)
	{
		if((*it)->getPlatformIds().empty())
		{
			mWindow->pushGui(new GuiMsgBox(mWindow, 
				...
				"EVET", std::bind(&GuiScraperStart::start, this), 
				"HAYIR", nullptr));
			return;
		}
	}
