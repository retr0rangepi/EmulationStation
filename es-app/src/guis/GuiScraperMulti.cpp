	...
	mSearchQueue(searches)
{
	...
	mSystem = std::make_shared<TextComponent>(mWindow, "SYSTEEM", Font::get(FONT_SIZE_MEDIUM), 0x777777FF, ALIGN_CENTER);
	...
	if(approveResults)
	{
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "INPUT", "zoeken", [&] {
			mSearchComp->openInputScreen(mSearchQueue.front());
			mGrid.resetCursor();
		}));

		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "OVERSLAAN", "skip", [&] {
			skip();
			mGrid.resetCursor();
		}));
	}
...
void GuiScraperMulti::doNextSearch()
{
	...
	// update subtitle
	ss.str(""); // clear
	ss << "GAME"<< (mCurrentGame + 1) << "VAN"<< mTotalGames << "-" << Utils::String::toUpper(Utils::FileSystem::getFileName(mSearchQueue.front().game->getPath()));
...
void GuiScraperMulti::finish()
{
	...
	{
		ss << "GEEN GAMES GESCRAPED";
	}else{
		ss << mTotalSuccessful << "SPEL"<< ((mTotalSuccessful > 1) ? "S" : "") << "";

		if(mTotalSkipped > 0)
			ss << "\n" << mTotalSkipped << "SPEL"<< ((mTotalSkipped > 1) ? "S" : "") << "OVERGESLAGEN";
	}
