	...
	mSearchQueue(searches)
{
	...
	// set up grid
	mTitle = std::make_shared<TextComponent>(mWindow, "ИДЁТ ЗАГРУЗКА ОПИСАНИЙ", Font::get(FONT_SIZE_LARGE), 0x555555FF, ALIGN_CENTER);
	mGrid.setEntry(mTitle, Vector2i(0, 0), false, true);

	mSystem = std::make_shared<TextComponent>(mWindow, "СИСЕМА", Font::get(FONT_SIZE_MEDIUM), 0x777777FF, ALIGN_CENTER);
	...
	if(approveResults)
	{
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "ВВОД", "поиск", [&] {
			mSearchComp->openInputScreen(mSearchQueue.front());
			mGrid.resetCursor();
		}));

		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "ПРОПУСТИТЬ", "skip", [&] {
			skip();
			mGrid.resetCursor();
		}));
	}

	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "СТОП", "stop (progress saved)", std::bind(&GuiScraperMulti::finish, this)));
...
void GuiScraperMulti::doNextSearch()
{
	...
	// update subtitle
	ss.str(""); // clear
	ss << "ИГРА"<< (mCurrentGame + 1) << " ИЗ "<< mTotalGames << " - " << Utils::String::toUpper(Utils::FileSystem::getFileName(mSearchQueue.front().game->getPath()));
...
void GuiScraperMulti::finish()
{
	...
	{
		ss << "НИ ОДНА ИГРА НЕ ЗАГРУЖЕНА.";
	}else{
		ss << mTotalSuccessful << " ИГРА"<< ((mTotalSuccessful > 1) ? "S" : "") << " УСПЕШНО ЗАГРУЖЕНО!";

		if(mTotalSkipped > 0)
			ss << "\n" << mTotalSkipped << " ИГРА"<< ((mTotalSkipped > 1) ? "S" : "") << "ПРОПУЩЕНО.";
	}
