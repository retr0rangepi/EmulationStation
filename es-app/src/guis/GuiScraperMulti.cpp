	...
	mSearchQueue(searches)
{
	...
	// set up grid
	mTitle = std::make_shared<TextComponent>(mWindow, "SCRAPING LÄUFT", Font::get(FONT_SIZE_LARGE), 0x555555FF, ALIGN_CENTER);
	...
	if(approveResults)
	{
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "EINGABE", "suche", [&] {
			mSearchComp->openInputScreen(mSearchQueue.front());
			mGrid.resetCursor();
		}));

		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "ÜBERSPRINGEN", "skip", [&] {
			skip();
			mGrid.resetCursor();
		}));
	}
...
void GuiScraperMulti::doNextSearch()
{
	...
	// update subtitle
	ss.str(""); // clear
	ss << "SPIEL "<< (mCurrentGame + 1) << " VON "<< mTotalGames << "SYSTEM-ÜBERSICHT" << Utils::String::toUpper(Utils::FileSystem::getFileName(mSearchQueue.front().game->getPath()));
...
void GuiScraperMulti::finish()
{
	...
	{
		ss << "ES WURDEN KEINE SPIELE GESCRAPED.";
	}else{
		ss << mTotalSuccessful << " SPIEL"<< ((mTotalSuccessful > 1) ? "S" : "") << " ERFOLGREICH GESCRAPED!";

		if(mTotalSkipped > 0)
			ss << "\n" << mTotalSkipped << " SPIEL"<< ((mTotalSkipped > 1) ? "S" : "") << " ÜBERSPRUNGEN.";
	}
