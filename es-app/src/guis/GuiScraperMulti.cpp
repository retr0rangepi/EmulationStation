	...
	mSearchQueue(searches)
{
	...
	// set up grid
	mTitle = std::make_shared<TextComponent>(mWindow, "ETİKETLEME İŞLENİYOR", Font::get(FONT_SIZE_LARGE), 0x555555FF, ALIGN_CENTER);
	mGrid.setEntry(mTitle, Vector2i(0, 0), false, true);

	mSystem = std::make_shared<TextComponent>(mWindow, "SİSTEM", Font::get(FONT_SIZE_MEDIUM), 0x777777FF, ALIGN_CENTER);
	...
	if(approveResults)
	{
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "GİR", "ara", [&] {
			mSearchComp->openInputScreen(mSearchQueue.front());
			mGrid.resetCursor();
		}));

		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "ATLA", "skip", [&] {
			skip();
			mGrid.resetCursor();
		}));
	}

	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "DURDUR", "stop (progress saved)", std::bind(&GuiScraperMulti::finish, this)));
...
void GuiScraperMulti::doNextSearch()
{
	...
	// update subtitle
	ss.str(""); // clear
	ss << "OYUN"<< (mCurrentGame + 1) << " NIN "<< mTotalGames << " - " << Utils::String::toUpper(Utils::FileSystem::getFileName(mSearchQueue.front().game->getPath()));
...
void GuiScraperMulti::finish()
{
	...
	{
		ss << "HİÇ BİR OYUN ETİKETLENMEDİ.";
	}else{
		ss << mTotalSuccessful << " OYUN"<< ((mTotalSuccessful > 1) ? "S" : "") << " BAŞARIYLA ETİKETLENDİ!";

		if(mTotalSkipped > 0)
			ss << "\n" << mTotalSkipped << " OYUN"<< ((mTotalSkipped > 1) ? "S" : "") << " ATLANDI.";
	}
