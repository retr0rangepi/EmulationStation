	...
	mSearchQueue(searches)
{
	...
	// set up grid
	mTitle = std::make_shared<TextComponent>(mWindow, "正在SCRAPE", Font::get(FONT_SIZE_LARGE), 0x555555FF, ALIGN_CENTER);
	mGrid.setEntry(mTitle, Vector2i(0, 0), false, true);

	mSystem = std::make_shared<TextComponent>(mWindow, "系統", Font::get(FONT_SIZE_MEDIUM), 0x777777FF, ALIGN_CENTER);
	...
	if(approveResults)
	{
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "輸入", "搜尋", [&] {
			mSearchComp->openInputScreen(mSearchQueue.front());
			mGrid.resetCursor();
		}));

		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "跳過", "skip", [&] {
			skip();
			mGrid.resetCursor();
		}));
	}

	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "停止", "stop (progress saved)", std::bind(&GuiScraperMulti::finish, this)));
...
void GuiScraperMulti::doNextSearch()
{
	...
	// update subtitle
	ss.str(""); // clear
	ss << "遊戲"<< (mCurrentGame + 1) << "OF"<< mTotalGames << "-" << Utils::String::toUpper(Utils::FileSystem::getFileName(mSearchQueue.front().game->getPath()));
...
void GuiScraperMulti::finish()
{
	...
	if(mTotalSuccessful == 0)
	{
		...
		ss << mTotalSuccessful << "遊戲"<< ((mTotalSuccessful > 1) ? "S" : "") << "成功SCRAPE";

		if(mTotalSkipped > 0)
			ss << "\n" << mTotalSkipped << "遊戲"<< ((mTotalSkipped > 1) ? "S" : "") << "跳過";
	}
