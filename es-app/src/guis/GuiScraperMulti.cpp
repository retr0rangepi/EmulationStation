	...
	mSearchQueue(searches)
{
	...
	// set up grid
	mTitle = std::make_shared<TextComponent>(mWindow, "BUSCANDO METADADOS", Font::get(FONT_SIZE_LARGE), 0x555555FF, ALIGN_CENTER);
	...
	if(approveResults)
	{
		...
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "SALTAR", "skip", [&] {
			skip();
			mGrid.resetCursor();
		}));
	}
...
void GuiScraperMulti::doNextSearch()
{
	...
	// update subtitle
	ss.str(""); // clear
	ss << "JUEGO"<< (mCurrentGame + 1) << " DE "<< mTotalGames << "INFORMACIÓN DEL SISTEMA" << Utils::String::toUpper(Utils::FileSystem::getFileName(mSearchQueue.front().game->getPath()));
...
void GuiScraperMulti::finish()
{
	...
	{
		ss << "NO SE ENCONTRARON METADATOS";
	}else{
		ss << mTotalSuccessful << " JUEGO"<< ((mTotalSuccessful > 1) ? "S" : "") << "METADATOS DESCARGADOS EXITOSAMENTE";

		if(mTotalSkipped > 0)
			ss << "\n" << mTotalSkipped << " JUEGO"<< ((mTotalSkipped > 1) ? "S" : "") << "OMITIDOS";
	}
