	...
	mSearchQueue(searches)
{
	...
	// set up grid
	mTitle = std::make_shared<TextComponent>(mWindow, "BUSCA EM PROGRESSO", Font::get(FONT_SIZE_LARGE), 0x555555FF, ALIGN_CENTER);
	mGrid.setEntry(mTitle, Vector2i(0, 0), false, true);

	mSystem = std::make_shared<TextComponent>(mWindow, "SISTEMA", Font::get(FONT_SIZE_MEDIUM), 0x777777FF, ALIGN_CENTER);
	...
	if(approveResults)
	{
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "EDITA", "BUSCA", [&] {
			mSearchComp->openInputScreen(mSearchQueue.front());
			mGrid.resetCursor();
		}));

		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "DESCARTA", "skip", [&] {
			skip();
			mGrid.resetCursor();
		}));
	}

	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "PARAR", "stop (progress saved)", std::bind(&GuiScraperMulti::finish, this)));
...
void GuiScraperMulti::doNextSearch()
{
	...
	// update subtitle
	ss.str(""); // clear
	ss << "JOGO "<< (mCurrentGame + 1) << " DE "<< mTotalGames << "INFORMAÇÃO DO SISTEMA" << Utils::String::toUpper(Utils::FileSystem::getFileName(mSearchQueue.front().game->getPath()));
...
void GuiScraperMulti::finish()
{
	...
	{
		ss << "NENHUMA INFORMAÇÃO FOI BAIXADA.";
	}else{
		ss << mTotalSuccessful << " JOGO"<< ((mTotalSuccessful > 1) ? "S" : "") << " ENCONTRADOS COM SUCESSO";

		if(mTotalSkipped > 0)
			ss << "\n" << mTotalSkipped << " JOGO"<< ((mTotalSkipped > 1) ? "S" : "") << "DESCARTADOS";
	}
