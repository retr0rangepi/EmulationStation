	...
	mSearchQueue(searches)
{
	...
	// set up grid
	mTitle = std::make_shared<TextComponent>(mWindow, "BUSCANDO...", Font::get(FONT_SIZE_LARGE), 0x555555FF, ALIGN_CENTER);
	mGrid.setEntry(mTitle, Vector2i(0, 0), false, true);

	mSystem = std::make_shared<TextComponent>(mWindow, "SISTEMAS", Font::get(FONT_SIZE_MEDIUM), 0x777777FF, ALIGN_CENTER);
	...
	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "DETENER", "stop (progress saved)", std::bind(&GuiScraperMulti::finish, this)));
...
void GuiScraperMulti::finish()
{
	...
	{
		ss << "NO SE ENCONTRARON DATOS";
	}else{
		ss << mTotalSuccessful << "JUEGO"<< ((mTotalSuccessful > 1) ? "S" : "") << "DESCARGA EXITOSA";

		if(mTotalSkipped > 0)
			ss << "\n" << mTotalSkipped << "JUEGO"<< ((mTotalSkipped > 1) ? "S" : "") << "DESCARTADOS";
	}
