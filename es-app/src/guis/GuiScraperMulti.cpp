	...
	mSearchQueue(searches)
{
	...
	// set up grid
	mTitle = std::make_shared<TextComponent>(mWindow, "Scraping kører", Font::get(FONT_SIZE_LARGE), 0x555555FF, ALIGN_CENTER);
	...
	if(approveResults)
	{
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "Input", "Søg", [&] {
			mSearchComp->openInputScreen(mSearchQueue.front());
			mGrid.resetCursor();
		}));

		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "Spring over", "skip", [&] {
			skip();
			mGrid.resetCursor();
		}));
	}
...
void GuiScraperMulti::doNextSearch()
{
	...
	// update subtitle
	ss.str(""); // clear
	ss << "spil"<< (mCurrentGame + 1) << "af"<< mTotalGames << "-" << Utils::String::toUpper(Utils::FileSystem::getFileName(mSearchQueue.front().game->getPath()));
...
void GuiScraperMulti::finish()
{
	...
	{
		ss << "Ingen spil blev scraped";
	}else{
		ss << mTotalSuccessful << "Spil"<< ((mTotalSuccessful > 1) ? "S" : "") << "sucess scraped";

		if(mTotalSkipped > 0)
			ss << "\n" << mTotalSkipped << "Spil"<< ((mTotalSkipped > 1) ? "S" : "") << "Sprunget over";
	}
