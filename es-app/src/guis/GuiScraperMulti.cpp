	...
	mSearchQueue(searches)
{
	...
	// set up grid
	mTitle = std::make_shared<TextComponent>(mWindow, "Scraping in corso", Font::get(FONT_SIZE_LARGE), 0x555555FF, ALIGN_CENTER);
	...
	if(approveResults)
	{
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "INPUT", "Cerca", [&] {
			mSearchComp->openInputScreen(mSearchQueue.front());
			mGrid.resetCursor();
		}));

		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "SALTA", "skip", [&] {
			skip();
			mGrid.resetCursor();
		}));
	}
...
void GuiScraperMulti::doNextSearch()
{
	...
	// update subtitle
	ss.str(""); // clear
	ss << "GIOCO"<< (mCurrentGame + 1) << "DI"<< mTotalGames << "INFORMAZIONI SISTEMA" << Utils::String::toUpper(Utils::FileSystem::getFileName(mSearchQueue.front().game->getPath()));
...
void GuiScraperMulti::finish()
{
	...
	{
		ss << "NESSUN GIOCO SCRAPED";
	}else{
		ss << mTotalSuccessful << "GIOCO"<< ((mTotalSuccessful > 1) ? "S" : "") << "SCRAPING COMPLETATO";

		if(mTotalSkipped > 0)
			ss << "\n" << mTotalSkipped << "GIOCO"<< ((mTotalSkipped > 1) ? "S" : "") << "SALTATO";
	}
