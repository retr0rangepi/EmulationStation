	...
	mSearchQueue(searches)
{
	...
	if(approveResults)
	{
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "输入", "查找", [&] {
			mSearchComp->openInputScreen(mSearchQueue.front());
			mGrid.resetCursor();
		}));

		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "跳过", "skip", [&] {
			skip();
			mGrid.resetCursor();
		}));
	}
...
void GuiScraperMulti::doNextSearch()
{
	...
	// update subtitle
	ss.str(""); // clear
	ss << "游戏"<< (mCurrentGame + 1) << "OF"<< mTotalGames << "-" << Utils::String::toUpper(Utils::FileSystem::getFileName(mSearchQueue.front().game->getPath()));
...
void GuiScraperMulti::finish()
{
	...
	if(mTotalSuccessful == 0)
	{
		...
		ss << mTotalSuccessful << "游戏"<< ((mTotalSuccessful > 1) ? "S" : "") << "成功SCRAPE";

		if(mTotalSkipped > 0)
			ss << "\n" << mTotalSkipped << "游戏"<< ((mTotalSkipped > 1) ? "S" : "") << "跳过";
	}
