...
GuiSlideshowScreensaverOptions::GuiSlideshowScreensaverOptions(Window* window, const char* title) : GuiScreensaverOptions(window, title)
{
	...
	addWithLabel(row, "МЕНЯТЬ ИЗОБРАЖЕНИЕ ПОСЛЕ (СЕК)", sss_image_sec);
	...
	addWithLabel(row, "РАСТЯГИВАТЬ ИЗОБРАЖЕНИЯ", sss_stretch);
	...
	addEditableTextComponent(row, "ФОНОВОЕ АУДИО", sss_bg_audio_file, Settings::getInstance()->getString("SlideshowScreenSaverBackgroundAudioFile"));
	...
	addWithLabel(row, "ИСПОЛЬЗОВАТЬ СВОИ ИЗОБРАЖЕНИЯ", sss_custom_source);
	...
	addEditableTextComponent(row, "ПАПКА СВОИХ ИЗОБРАЖЕНИЙ", sss_image_dir, Settings::getInstance()->getString("SlideshowScreenSaverImageDir"));
	...
	addWithLabel(row, "ПРОСМАТРИВАТЬ РЕКУРСИВНО", sss_recurse);
	...
	addEditableTextComponent(row, "ФИЛЬТР СВОИХ ИЗОБРАЖЕНИЙ", sss_image_filter, Settings::getInstance()->getString("SlideshowScreenSaverImageFilter"));
