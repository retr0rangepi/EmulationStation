...
GuiSlideshowScreensaverOptions::GuiSlideshowScreensaverOptions(Window* window, const char* title) : GuiScreensaverOptions(window, title)
{
	...
	addWithLabel(row, "Skift image efter (sek.)", sss_image_sec);
	...
	addWithLabel(row, "Stræk image", sss_stretch);
	...
	addEditableTextComponent(row, "baggrunds lyd", sss_bg_audio_file, Settings::getInstance()->getString("SlideshowScreenSaverBackgroundAudioFile"));
	...
	addWithLabel(row, "benyt custom image", sss_custom_source);
	...
	addEditableTextComponent(row, "custom image mappe", sss_image_dir, Settings::getInstance()->getString("SlideshowScreenSaverImageDir"));
	...
	addWithLabel(row, "custom image mappe rekursiv", sss_recurse);
