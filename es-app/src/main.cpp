...
bool loadSystemConfigFile(const char** errorString)
{
	*errorString = NULL;
	...
	if(!SystemData::loadConfig())
	{
		...
		*errorString = "ПОХОЖЕ, ЧТО ФАЙЛ КОНФИГУРАЦИИ СИСТЕМ НЕ СОЗДАН ИЛИ ПОВРЕЖДЁН. ВАМ НУЖНО СОЗДАТЬ ЕГО ВРУЧНУЮ.\n\n"
			"ДОПОЛНИТЕЛЬНАЯ ИНФОРМАЦИЯ - НА САЙТЕ EMULATIONSTATION.ORG";
		return false;
	}
	...
	if(SystemData::sSystemVector.size() == 0)
	{
		...
		*errorString = "НЕ МОГУ НАЙТИ НИ ОДНОЙ СИСТЕМЫ!\n"
			"ПРОВЕРЬТЕ, ЧТО ПУТИ ПРАВИЛЬНО ПРОПИСАНЫ В ФАЙЛЕ КОНФИГУРАЦИИ"
